...
int main() {
    ...
    while(t--) {
        ...
        string s; cin >> s;
        lli ans = LLONG_MAX;
        ans = min(ans,cnt("RGB",s));
        ans = min(ans,cnt("RBG",s));
        ans = min(ans,cnt("GRB",s));
        ans = min(ans,cnt("GBR",s));
        ans = min(ans,cnt("BRG",s));
        ans = min(ans,cnt("BGR",s));
        cout << ans << endl;
    }
    return 0;
}