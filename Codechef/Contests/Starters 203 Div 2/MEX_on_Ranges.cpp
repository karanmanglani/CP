...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> wanted;
        for(lli i = 0;i <= n;i++){
            if(s[i] == '1') wanted.push_back(i);
        }
        vector<lli> ans;
        ans.push_back(0);
        lli i = 2;
        while(i < wanted.size()){
            if(wanted[i] - wanted[i-1] == 1){
                ans.push_back(wanted[i-1]);
            }else{
                lli mx = wanted[i] - 1;
                lli mn = wanted[i-1];
                for(lli j = mx;j >= mn;j--) ans.push_back(j);
            }
            i++;
        }

        lli last = ans.back();
        if(wanted.back() == 1) last = 2;
        while(ans.size() < n) ans.push_back(last);
