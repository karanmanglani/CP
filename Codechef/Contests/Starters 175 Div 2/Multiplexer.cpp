...
int main() {
    ...
    while(t--) {
        ...
        for(auto i : mp){
            ...
            if(i.first % x == 0 and mp.find(i.first/x) != mp.end()){
                vector<lli> b = i.second,c = mp[i.first/x];
                if(b == c) {ans = (b.size() > ans) ? b.size() : ans;continue;}
                vector<lli> d;
                lli y = 0, z = 0;
                while(y < b.size() and z < c.size()){
                    if(b[y] < c[z]){
                        d.push_back(b[y]);
                        y++;
                    }else{
