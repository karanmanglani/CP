...
int main() {
    ...
    while(t--) {
        ...
        auto f = [&](lli i, lli j , auto &&F) -> lli{
            ...
            dp[i][j] = 0;
            if(i -1 >= 0) dp[i][j] = (dp[i][j] + F(i-1,j,F)) % MOD;
            if(i-1 >= 0 && j-1 >= 0) dp[i][j] = (dp[i][j] + F(i-1,j-1,F)) % MOD;
            if(i == k and j == k){
                dp[i][j] = (dp[i][j] - 1 + MOD) % MOD;
            }else if(i-k-1 >= 0 && j-k >= 0) dp[i][j] = (dp[i][j] - F(i-k-1,j-k,F) + MOD) % MOD;
            return dp[i][j];
        };
        ...
        for(lli i = 0;i <= n;i++){
            lli ways = f(n,i,f);
            ways = mod_mul(ways, fct[i], MOD);
            ways = mod_mul(ways , fct[n-i],MOD);
            ans = (ans + ways) % MOD;
        }
