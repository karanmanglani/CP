...
int main() {
    ...
    while(t--) {
        ...
        bool ans = true;
        for(lli i = 0;i < n;i++){
            while(curr < n && f[curr] == 0) curr++;
            if(curr == n) break;
            if(a[i] == -1) a[i] = curr + 1;
            f[curr]--;
        }

        if(*min_element(f.begin(),f.end()) == -1) ans = false;
        vector<lli> p(n,0), s(n,0);
        vector<lli> temp(n,0);
        for(lli i = 0;i < n;i++){
            p[i] = (i == 0 ? a[i] : max(p[i-1],a[i]));
            temp[p[i]-1]++;
        }

        for(lli i = n-1;i >= 0;i--){
            s[i] = (i == n-1 ? a[i] : min(s[i+1],a[i]));
            temp[s[i]-1]++;
        }
        
        for(lli i = 0;i <n;i++){
            if(temp[i] != ff[i]) {ans = false;break;}
        }
