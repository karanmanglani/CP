...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> p(n,0),w(n-1);
        for(lli i = 1;i < n;i++) cin >> p[i];
        for(auto &i : w) cin >> i;
        for(lli i = 1;i < n;i++){
            adj[p[i]-1].push_back({i,w[i-1]});
        }

        // Finding lca of all nodes given 
        auto findLCA = [&](vector<bool> &nodes) -> lli{
            lli ans = 0;
            lli sz = count(nodes.begin(), nodes.end(),true);
            bool fnd = false;
            auto dfs = [&](lli node, bool &fnd,auto&& self) -> lli{
                lli cnt =  nodes[node] ? 1 : 0;
                for(auto &child : adj[node]){
                    cnt += self(child.first, fnd , self);
                }
                if(!fnd && cnt == sz){
                    ans = node;
                    fnd = true;
        ...
        // dp[i][j] => maximum water reachable till node i if j(0 or 1) pipes are fixed in path from root to i
        dp[0][0] = dp[0][1] = k;
        auto fillDp = [&](lli node,lli wt ,lli p, auto &&self) -> void{
            if(p != -1){
                // No pipes fixed till now
                dp[node][0] = dp[p][0] - (dp[p][0]/wt);

                // One pipe fixed till now including this edge
                dp[node][1] = max(dp[p][1] - (dp[p][1]/wt), dp[p][0]);
            }

            for(auto &child : adj[node]){
                self(child.first, child.second, node, self);
            }
        };
        fillDp(0,0,-1,fillDp);

        auto chk = [&](lli md) -> bool{
            vector<bool> nodes(n, false);
            lli wtt = 0;
            auto dfs2 = [&](lli node,lli wtr,auto &&self) -> void{
                for(auto &child : adj[node]){
                    self(child.first, wtr -( wtr/child.second), self);
                }
                if(wtr < md) nodes[node] = true;
            };
            dfs2(0,k,dfs2);
            if(count(nodes.begin(), nodes.end(), true) == 0) return true;
            lli lca = findLCA(nodes);
            bool fnd = true;
            auto dfs3 = [&](lli node, lli wtr, auto &&self) -> void{
                if(wtr < md) fnd = false;
                for(auto &child : adj[node]){
                    self(child.first,wtr - (wtr/child.second), self);
                }
            };
            dfs3(lca,dp[lca][1],dfs3);
            return fnd;
        };
