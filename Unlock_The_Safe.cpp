...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> diff;
        for(lli i = 0;i < n;i++){
            lli x = min(abs(a[i]-b[i]), 9-a[i]+b[i]);
            x = min(x, 9-b[i]+a[i]);
            cnt += x;
            if(9 - 2*x >= 0) diff.push_back(9 - 2*x);
            else diff.push_back(2*x - 9);
        }
        if(cnt > k){
            cout << "NO" << endl;
        }else if((k - cnt) % 2 == 0 || (k -cnt)%9 == 0){
            cout << "YES" << endl;
        }else{
            bool flag = false;

            for(auto i : diff) if(i <= abs(k-cnt) and (i & 1)) flag = true;
