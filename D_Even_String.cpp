...
// **********************************************************************************************
auto pcfres = pcf(5e5);
vector<lli> fct = pcfres.first;
vector<lli> ifct = pcfres.second;
lli solve(lli i, vector<lli>& a, lli& ans,lli opos,lli epos) {
    if(i == 26){return 1;}
    if(a[i] == 0) return solve(i+1,a,ans,opos,epos);
    lli eve = -1, odd = -1;
    if(a[i] <= epos){
        eve = ncrf(epos,a[i],fct,ifct);
        eve =( eve % MOD * solve(i+1,a,ans,opos,epos-a[i]) % MOD)%MOD; 
    }
    if(a[i] <= opos){
        odd = ncrf(opos,a[i],fct,ifct);
        odd = (odd % MOD * solve(i+1,a,ans,opos-a[i],epos) % MOD)%MOD;
    }
    if(eve == -1 && odd == -1) return 0;
    else if(eve == -1) return odd;
    else if(odd == -1) return eve;
    else return (eve + odd) % MOD;
}
...
int main() {
    ...
    while(t--) {
        ...
        lli opos = (n+1)/2, epos = n/2;
        vector<lli> dp(opos+1, 0), ndp(opos+1, 0);
        dp[0] = 1;
        lli tot = 0;
        for(int i = 0; i < 26; i++){
            if(a[i] == 0){
            ...
            for(int j = 0; j <= opos; j++){
                if(dp[j] == 0) continue;
                if(j + a[i] <= opos)
                    ndp[j + a[i]] = (ndp[j + a[i]] + dp[j] * ncrf(opos - j, a[i], fct, ifct)) % MOD;
                if(tot - j + a[i] <= epos)
                    ndp[j] = (ndp[j] + dp[j] * ncrf(epos - (tot - j), a[i], fct, ifct)) % MOD;
            }
            tot += a[i];
            dp = ndp;
        }
        cout << dp[opos] % MOD << endl;
    }
    return 0;
}