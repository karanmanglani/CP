...
int main() {
    ...
    lli t; cin >> t;
    lli INF = 1e18;
    vector<vector<pair<int,int>>> r(26);

    auto getR = [&](int s) -> vector<pair<int,int>> {
    ...
    while(t--) {
        lli n;cin >> n;
        string s;cin >> s;
        vector<vector<lli>> dp(n, vector<lli>(27, INF));
        auto tag = [&](char c) -> int { return (c == 'z' ? 0 : c - 'a' + 1); };
        int st = tag(s[0]);
        for(auto &p : r[st]){
            dp[0][p.first] = min(dp[0][p.first], (lli)p.second);
        }
        for(int i = 1; i < n; i++){
            int ct = tag(s[i]);
            vector<lli> pref(27, INF);
            pref[1] = dp[i-1][1];
            for(int v = 2; v <= 26; v++){
                pref[v] = min(pref[v-1], dp[i-1][v]);
            }
            for(auto &p : r[ct]){
                int ord = p.first, cost = p.second;
                if(pref[ord] < INF)
                    dp[i][ord] = min(dp[i][ord], pref[ord] + cost);
            }
        }
        lli ans = INF;
        for(int v = 1; v <= 26; v++){
