...
bool chk(lli md, lli n, lli c,lli d, vector<lli> &p){
    lli x = d/(md + 1);
    lli y = d%(md + 1);
    lli len = min(n,md+1);
    lli sm = p[len-1];
    y = min(y,n);
    lli ans = (x * sm) + (y > 0 ? p[y-1] : 0);
    return ans >= c;
}
...
int main() {
    ...
    while(t--) {
        lli n,c,d;cin >> n >> c >> d;
        vector<lli> a(n),p(n);for(auto &i : a)cin >> i;
        sort(a.rbegin(),a.rend());
        p[0] = a[0];
        for(int i = 1;i < n;i++)p[i] = p[i-1] + a[i];
        lli sm = accumulate(a.begin(), a.begin() + min(d,n), 0LL);
        if(sm >= c){
            cout << "Infinity" << endl;
            continue;
        }
