...
int main() {
    ...
    while(t--) {
        lli n,k;cin >> n >> k;
        vector<pair<lli,pair<lli,lli>>> a(n);
        for(lli i = 0; i < n; i++){
        ...
        for(lli i = 0; i <= 200; i++){
            vector<lli> curr;
            for(lli j = 0; j < n; j++){
                if(!r[j] && a[j].first <= i && a[j].second.first >= i)
                    curr.push_back(j);
            }
            if(curr.size() <= k) continue;
            while(curr.size() > k){
                lli x = curr[0];
                for(lli idx = 1; idx < curr.size(); idx++){
                    if(a[curr[idx]].second.first > a[x].second.first)
                        x = curr[idx];
                }
                r[x] = true;
                ans.push_back(a[x].second.second);
                vector<lli> tmp;
                for(auto v : curr)
                    if(v != x) tmp.push_back(v);
                curr = tmp;
            }
        }
        cout << ans.size() << "\n";
