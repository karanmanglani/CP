...
int main() {
    ...
    while(t--) {
        ...
        int n0=count(t.begin(), t.end(),'0'), n1=t.size()-n0;
        string p="";
        for(auto ch: t) p.push_back(ch=='0'?'a':'b');
        Hashing H(p);
        int l=0, n=t.size();
        for(int i=1;i<n;i++){
            vector<lli> a=H.substringHash(0,i-1), b=H.substringHash(n-i,n-1);
            if(a==b) l=i;
        }
        string ext=t.substr(l);
        int e0=count(ext.begin(), ext.end(),'0'), e1=ext.size()-e0;
        string r="";
        if(c0>=n0 && c1>=n1){
            r+=t; c0-=n0; c1-=n1;
            while(!ext.empty() && c0>=e0 && c1>=e1){
                r+=ext; c0-=e0; c1-=e1;
            }
        }
        while(c0--) r.push_back('0');
        while(c1--) r.push_back('1');
        cout << r;
    }
    return 0;
}