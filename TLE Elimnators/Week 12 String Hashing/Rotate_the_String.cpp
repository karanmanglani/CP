...
struct Hashing {
    ...
    Hashing(string a) {
        ...
        for(lli i = 0; i < primes; i++) {
            ...
            for(lli j = 0; j < n; j++) {
                lli idx = (s[j] >= 'a' && s[j] <= 'z') ? s[j] - 'a' + 1 : s[j] - 'A' + 27;
                hashValues[i][j] = ((idx + 1LL) * powersOfBase[i][j]) % hashPrimes[i];
    ...
    // Hashing specific to this question 
    vector<lli> kRotatedHash(lli ns){
        vector<lli> ansHash(primes);
        if(ns == 0) return substringHash(0,n-1);
        vector<lli> h1 = this->substringHash(ns,n-1);
        vector<lli> h2 = this->substringHash(0,ns-1);
        for(lli i = 0;i < primes;i++){
            ansHash[i] = (h1[i] + (powersOfBase[i][n-ns] * h2[i])) % hashPrimes[i];
        }
        return ansHash;
    }
};
...
int main() {
    ...
    while(t--) {
        ...
        lli n = s.length();
        Hashing h(s);
        vector<lli> h1 = h.substringHash(0,n-1);
        lli ns = 0;
        bool flag = true;
        lli cnt = 0;
        while(true){
            ns += flag ? m : p;
            flag = !flag;
            vector<lli> temp = h.kRotatedHash(ns%n);
            cnt++;
            if(temp == h1){
                cout << cnt << endl;
                break;
