...
int main() {
    ...
    while(t--) {
        ...
        lli n = s.length();
        lli ans = 0;
        for(lli i = 1;i <=( n-2)/2;i++){
            vector<lli> h1 = h.substringHash(0,i-1);
            vector<lli> h2 = h.substringHash(i,2*i-1);
            lli nl = n - 2 * i;
            if(nl & 1) continue;
            nl /= 2;
            vector<lli> h3 = h.substringHash(2 * i, 2 * i + nl - 1);
            vector<lli> h4 = h.substringHash(2 * i + nl, n - 1);
            if(h1 == h2 and h3 == h4) ans++;
