...
int main() {
    ...
    while(t--) {
        ...
        lli bp = -1, bs = -1;
        for (lli i = 0; i < mlen; i++) {
            if (h.substringHash(ll, ll + i) == hr.substringHash(n - 1 - (ll + i), n - 1 - ll))
                bp = i;
        }
        for (lli i = 0; i < mlen; i++) {
            if (h.substringHash(rr - i, rr) == hr.substringHash(n - 1 - rr, n - 1 - (rr - i)))
                bs = i;
        }
        ...
        if (bp >= 0 || bs >= 0) {
            if (bp >= bs)
                mid = s.substr(ll, bp + 1);
            else
                mid = s.substr(rr - bs, bs + 1);
        }
