...
int main() {
    ...
    while(t--) {
        ...
        while(st <= e){
            lli md = st + (e - st) / 2;
            if([&]{
                map<vector<lli> , lli> fc;
                for(lli i = 0;i < n - md + 1;i++){
                    fc[h.substringHash(i, i + md - 1)]++;
                    if(fc[h.substringHash(i, i + md - 1)] > 1){
                        return true;
                    }
                }
                return false;
        ...
        map<vector<lli> , lli> fc;
        for(lli i = 0;i < n - ans + 1;i++){
            fc[h.substringHash(i, i + ans - 1)]++;
            if(fc[h.substringHash(i, i + ans - 1)] > 1){
