...
int main() {
    ...
    // your code goes here
    // lli t; cin >> t;
    auto spf = primeArr(1e5+1).second.first;
    lli t = 1;
    while(t--) {
        lli n,m;cin >> n >> m;
        ...
        while(m--){
            ...
            if(ch == '+'){
                ...
                for(auto i : y){
                    if(a[i.first].size() > 0) {
                        cout << "Conflict with " << *a[i.first].begin() << endl;
                        fnd = true;
                        break;
                    }
                }
                if(!fnd) {
                    for(auto i : y) a[i.first].insert(x);
                    cout << "Success" << endl;
                    on[x] = true;
                }
            }else {
                if(on[x]){
                    for(auto i : y) a[i.first].erase(a[i.first].find(x));
                    on[x] = false;
                    cout << "Success" << endl;
                }else{
