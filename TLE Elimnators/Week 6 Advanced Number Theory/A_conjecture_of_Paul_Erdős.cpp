...
int main() {
    ...
    // your code goes here
    lli t; cin >> t;
    auto ip = primeArr(10000000).first;
    set<lli> st;
    for(lli i = 1;i *i < 10000000;i++){
        for(lli j = 1;(j *j *j *j) < (10000000 - i*i);j++){
            lli x = i*i;
            lli y = j*j*j*j;
            if(x + y > 10000000) break;
            if(ip[x+y]) st.insert(x+y);
        }
    }
    vector<lli> a;
    ...
    while(t--) {
        lli n;cin >> n;
        lli ans = upper_bound(a.begin(),a.end(),n) - a.begin();
        cout << ans << endl;
    }
    return 0;
}