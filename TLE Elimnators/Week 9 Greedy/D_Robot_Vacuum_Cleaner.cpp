...
int main() {
    ...
    while(t--) {
        ...
        vector<pair<string,pair<lli,lli>>> b;
        vector<string> c,d;
        for(auto i : a){
            lli cnts = 0, cnth = 0;
            for(auto j : i){
            ...
            if(cnts == 0) d.push_back(i);
            else if(cnth == 0) c.push_back(i);
            else b.push_back({i,{cnts,cnth}});
        }
        sort(b.begin(),b.end(),[](pair<string,pair<lli,lli>> a,pair<string,pair<lli,lli>> b){
            return a.second.first * b.second.second > a.second.second * b.second.first;
        });
        string ns = "";
        for(auto i : c) ns += i;
        for(auto i : b) ns += i.first;
        for(auto i : d) ns += i;
        lli cnt = 0;
        lli ans = 0;
        for(char i : ns){
