...
int main() {
    ...
    while(t--) {
        ...
        auto dijkstra = [&](lli src) -> vector<vector<lli>>{
            vector<vector<lli>> dist(n,vector<lli>(2,LLONG_MAX));
            dist[src][a[src]] = 0;
            priority_queue<tuple<lli,lli,lli>, vector<tuple<lli,lli,lli>>, greater<>> pq;
            pq.push({0, src, a[src]});
            while(!pq.empty()){
                auto x = pq.top();
                lli d = get<0>(x);
                lli node = get<1>(x);
                lli hasHorse = get<2>(x);
                pq.pop();

                if(d > dist[node][hasHorse]) continue;
                for(auto &i : adj[node]){
                    lli next = i.first, w = i.second;
                    lli nextHasHorse = hasHorse | a[next];
                    lli cost = (hasHorse ? w/2 : w);
                    if(dist[next][nextHasHorse] > d + cost){
                        dist[next][nextHasHorse] = d + cost;
                        pq.push({dist[next][nextHasHorse], next, nextHasHorse});
                    }
                }
            }
            return dist;
        };
        auto robin = dijkstra(0), marian = dijkstra(n - 1);
        lli ans = LLONG_MAX;
        for(lli i = 0;i < n;i++){
