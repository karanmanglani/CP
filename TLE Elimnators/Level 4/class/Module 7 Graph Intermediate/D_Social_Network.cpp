...
class DisjointSet {
    ...
    void ubs(lli u, lli v) {
        ...
        if(pu != pv) {
            ...
            sizez.erase(sizez.find(size[pu]));
            sizez.erase(sizez.find(size[pv]));
            size[pu] += size[pv];
            sizez.insert(size[pu]);
            mxSz = max(mxSz, size[pu]);
            size[pv] = pu;
            parent[pv] = pu;
        }else extra++;
    }
...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0;i < d;i++){
            ...
            dsu.ubs(u,v);
            lli ans = -1;
            auto it = dsu.sizez.end();
            it--;
            for(lli i = 0;((i < dsu.extra + 1) && (i < dsu.sizez.size()));i++){
                ans += *it;
                it--;
            }
            cout << ans<< endl;
