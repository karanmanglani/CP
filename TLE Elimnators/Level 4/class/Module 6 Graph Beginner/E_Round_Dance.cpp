...
int main() {
    ...
    while(t--) {
        lli n;cin >>  n;
        vector<lli> a(n);for(auto &i : a) {cin >> i;i--;}
        vector<set<lli>> adj(n);
        for(lli i = 0; i < n; i++) {
            adj[a[i]].insert(i);
            adj[i].insert(a[i]);
        }
        ...
        auto bfs = [&](lli src){
            ...
            while(!q.empty()) {
                lli node = q.front();
                q.pop();
                if(adj[node].size() == 1) joins++;
                else if(adj[node].size() == 0) joins += 2;
            ...
            if(joins>0) jts.push_back(joins);
        };
        for(lli i = 0; i < n; i++) {
            if(!vis[i]) {
                bfs(i);
        ...
        sort(jts.rbegin(), jts.rend());
        lli l = 0, r = 1;

        while(r < jts.size()){
            while(l < r && jts[l] == 0) l++;
            if(l == r) r++;
            if(r < jts.size() && jts[l] > 0 && jts[r] > 0) {
               jts[l]--;
               jts[r]--;
               cnt++;
            }
        ...
        lli mn = comp - cnt, mx = comp;
        cout << max(mn,1LL) << " " << mx << endl;
    }
    return 0;
}