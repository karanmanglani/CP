...
int main() {
    ...
    while(t--) {
        ...
        lli n = ul.length();
        lli sd;cin >> sd;
        vector<vector<vector<lli>>> dp(n,vector<vector<lli>> (2, vector<lli> (sd, -1)));
        auto f = [&](lli idx, lli tight , lli sum,auto &&F) -> lli{
            if(idx == n){
                return (sum == 0) ? 1 : 0;
            }
            if(dp[idx][tight][sum] != -1) return dp[idx][tight][sum];
            lli ub = tight ? ul[idx] - '0' : 9;
            lli ans = 0;
            for(lli i = 0;i <= ub;i++){
                ans += F(idx + 1, tight & (i == ub ? 1 : 0), (sum + i)%sd, F);
                ans %= MOD;
            }
            return dp[idx][tight][sum] = ans;
        };
        cout << (f(0, 1, 0, f) - 1 + MOD) % MOD<< endl;
    }
    return 0;
}