...
int main() {
    ...
    while(t--) {
        ...
        // state : dp[i][mask] = number of ways of pairing up for ith person with mask representing the set of people already paired up
        vector<lli> dp(1 << n,-1);
        auto f = [&](lli mask, auto &&F) -> lli{
            lli index = __builtin_popcount(mask);
            if(index == n){
            ...
            lli ans = 0;
            for(lli i = 0;i < n;i++){
                if(mask & (1 << i) || a[index][i] == 0) continue;
                ans += F(mask | (1 << i), F);
                ans %= MOD;
            }
            return dp[mask] = ans;
