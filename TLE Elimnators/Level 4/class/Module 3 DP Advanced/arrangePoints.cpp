...
int main() {
    ...
    while(t--) {
        ...
        auto dist = [&](pair<lli, lli> &a, pair<lli, lli> &b) -> lli {
            return abs(a.first - b.first) + abs(a.second - b.second);
        };
        
        auto f = [&](lli prev, lli mask, auto &&F) -> lli{
            ...
            if(index == 0){
                ...
                for(lli i = 0;i < n ;i++){
                    ...
                    ans = min(ans , dist(points[prev], points[i]) + F(i,mask | (1 << i), F));
                }
