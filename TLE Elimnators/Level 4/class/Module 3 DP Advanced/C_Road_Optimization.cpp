...
int main() {
    ...
    while(t--) {
        lli n,l,k;cin >> n >> l >> k;
        vector<lli> d(n);for(auto &i : d) cin >> i;
        vector<lli> a(n);for(auto &i : a) cin >> i;
        // dp[i][j][z] = minimum time to reach city starting for city i with removed limits j and last non removed limit index being z
        // vector<vector<vector<lli>>> dp(n+1, vector<vector<lli>>(k + 1, vector<lli>(n, -1)));
        // auto f = [&](lli i, lli j, lli z , auto &&F){
        ...
        //     lli dist = d[i] - d[i-1];
        //     lli time = dist * speed;
        //     // case 1 : do no remove this limit
        //     dp[i][j][z] = min(dp[i][j][z] , time + F(i+1, j,i,F));
        //     // case 2 : remove this limit
        //     if(j < k){
        //         dp[i][j][z] = min(dp[i][j][z], time + F(i+1,j+1,z,F));
        //     }
        //     return dp[i][j][z];
        // };

        // Tabulation
        // // vector<vector<vector<lli>>> dp(n+1, vector<vector<lli>>(k + 1, vector<lli>(n, LLONG_MAX)));
        ...
        //             lli time = dist * speed;
        //             // case 1 : do no remove this limit
        //             dp[i][j][z] = min(dp[i][j][z] , time + dp[i+1][j][i]);
        //             // case 2 : remove this limit
        //             if(j < k){
        //                 dp[i][j][z] = min(dp[i][j][z], time + dp[i+1][j+1][z]);
        //             }
        //         }
        //     }
        // }
        ...
        // space optimization
        vector<vector<lli>> next(k + 1, vector<lli>(n, LLONG_MAX));
        for(lli j = 0;j <= k;j++){
            for(lli z = 0;z < n;z++){
                next[j][z] = (l - d[n-1]) * a[z];
            }
        }
        for(lli i = n-1;i > 0;i--){
            vector<vector<lli>> curr(k + 1, vector<lli>(n, LLONG_MAX));
            for(lli j = 0;j <= k;j++){
                for(lli z = 0;z < i;z++){
                    lli speed = a[z];
                    lli dist = d[i] - d[i-1];
                    lli time = dist * speed;
                    // case 1 : do no remove this limit
                    curr[j][z] = min(curr[j][z] , time + next[j][i]);
                    // case 2 : remove this limit
                    if(j < k){
                        curr[j][z] = min(curr[j][z], time + next[j+1][z]);
                    }
                }
            }
            next = curr;
