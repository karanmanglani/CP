...
int main() {
    ...
    while(t--) {
        ...
        sort(req.begin(), req.end(), [](auto &a, auto &b) { return a[1] < b[1]; });
        vector<lli> dp(1 << m, LLONG_MAX);
        dp[0] = 0;
        lli ans = LLONG_MAX;
        for(auto &p : req) {
            auto ndp = dp;
            for(lli mask = 0; mask < (1 << m); mask++) {
                if(dp[mask] == LLONG_MAX) continue;
                lli nmask = mask | p[2];
                if(ndp[nmask] > dp[mask] + p[0]) ndp[nmask] = dp[mask] + p[0];
            }
            dp = ndp;
            if(dp[(1 << m) - 1] != LLONG_MAX) {
                ans = min(ans, dp[(1 << m) - 1] + p[1] * b);
            }
        }
