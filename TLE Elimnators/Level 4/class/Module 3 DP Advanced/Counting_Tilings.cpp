...
int main() {
    ...
    while(t--) {
        lli n,m; cin >> n >> m;
        vector<vector<lli>> dp(m + 1, vector<lli>(1 << n, -1));
        vector<vector<lli>> transitions(1 << n);
        auto generateTransitions = [&](lli index, lli mask1, lli mask2 , auto &&self){
            if(index > n) return ;
            if(index == n) {
            ...
            // case 2 : vertical tile 
            self(index + 1, mask1 , mask2 |(1 << index), self);
            // case 3 : horizontal tile
            self(index + 2, mask1 , mask2 , self);
        };
        generateTransitions(0, 0, 0, generateTransitions);

        auto countWays = [&](lli index , lli mask, auto && self) -> lli{
            if(index == m) return mask == 0 ? 1LL : 0LL;
            if(dp[index][mask] != -1) return dp[index][mask];
            dp[index][mask] = 0;
            for(auto transition : transitions[mask]){
                dp[index][mask] = (dp[index][mask] + self(index + 1, transition, self)) % MOD;
            }
            return dp[index][mask];
        };

        cout << countWays(0, 0, countWays) << endl;
