...
int main() {
    ...
    while(t--) {
        lli n,q;cin >> n >> q;
        vector<vector<lli>> parent(n+1, vector<lli> (20, -1));
        ...
        vector<lli> dep(n+1,0);

        auto dfs = [&](lli node,lli p, auto&& self) -> void{
            parent[node][0] = p;
            for(lli i = 1;i < 20;i++){
                if(parent[node][i-1] != -1) parent[node][i] = parent[parent[node][i-1]][i-1];
            }
            if(parent[node][0] != -1) dep[node] = dep[parent[node][0]] + 1;
        ...
        auto findKthParent = [&](lli node, lli k) -> lli{
            lli curr = node;
            for(lli i = 0;i < 20 && curr != -1;i++){
                if(k & (1LL << i)) curr = parent[curr][i];
            }
        ...
        auto lca = [&](lli u, lli v) -> lli{
            if(u == v) return u;
            if(dep[u] < dep[v]) swap(u,v);
            lli diff = dep[u] - dep[v];
