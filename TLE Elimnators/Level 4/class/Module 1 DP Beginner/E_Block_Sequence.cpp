...
int main() {
    ...
    while(t--) {
        ...
        // state dp[i] -> minimum elements we can remove from i to n-1
        // transition dp[i] = min(1 + dp[i+1], dp[i+a[i]+1])
        vector<lli> dp(n+1,0);
        dp[n-1] = 1;
        for(lli i = n-2;i >= 0;i--){
            if(i + a[i] < n){
                dp[i] =(i + a[i] + 1 < n) ?  min(1LL + dp[i+1] , dp[i + a[i] + 1]) : min(1LL + dp[i+1],0LL);
            }else{
                dp[i] = 1 + dp[i+1];
            }
        }
