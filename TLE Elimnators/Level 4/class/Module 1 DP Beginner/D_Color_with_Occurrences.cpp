...
int main() {
    ...
    while(t--) {
        ...
        // state : dp[i] = minimum number of operations to make the characters s[i] to s[n-1] red
        // transition : dp[i] = min(1 + dp[i + a[j].size()]) for all j from 1 to n
        vector<lli> dp(s.size(),0);
        vector<lli> op(s.size(),-1);
        for(lli i = s.size()-1;i >= 0;i--){
            lli val = LLONG_MAX;
            for(lli j = 0;j < n;j++){
                if(i + a[j].size() <= s.size() and s.substr(i,a[j].size()) == a[j]){
                    if(i + a[j].size() == s.size()){
                        val = min(val,1LL);
                        op[i] = j;
                    }else{
                        lli x=LLONG_MAX;
                        for(lli k = i+1;k <= i + a[j].size();k++){
                            if(dp[k] != -1) x = min(x,dp[k]);
                        }
                        if(x != LLONG_MAX){
                            val = min(val,1LL + x);
                            if(val == 1LL + x){
        ...
        if(dp[0] != -1){
            ...
            while(i < s.size()){
                lli opr = op[i];
                ans.push_back({opr+1,i+1});
                lli mn = LLONG_MAX;
                bool flag = false;
                lli ni = i + a[opr].size();
                for(lli j = i + 1;j <= i + a[opr].size();j++){
                    if(j == s.size()) continue;
                    if(dp[j] != -1 and dp[j] <= mn){
                        mn = dp[j];
                        flag = true;
                        ni = j;
                    }
