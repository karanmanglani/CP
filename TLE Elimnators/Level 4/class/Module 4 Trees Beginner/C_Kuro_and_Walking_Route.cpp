...
int main() {
    ...
    while(t--) {
        ...
        auto dfs1 = [&](lli node, lli parent, auto&& self) -> bool {
            if(node == x) return true;
            vis++;
            bool flag = false;
            for(auto child : adj[node]) {
                if(child != parent) {flag |= self(child, node, self);hasX[child] = flag;}
            }
            return flag;
        };
        lli vis1 = 0;
        auto dfs2 = [&](lli node, lli parent, auto&& self) -> void{
            if(node == x) return ;
            vis1++;
            for(auto child : adj[node]){
                if(child != parent) self(child, node, self);
            }
        };
        dfs1(y,-1,dfs1);
        lli nv = n - vis;
        for(auto child : adj[y]){
            if(hasX[child]) {dfs2(child,y,dfs2);break;}
        }
        vis -= vis1;
        ans -= vis * nv;
        cout << ans << endl;
