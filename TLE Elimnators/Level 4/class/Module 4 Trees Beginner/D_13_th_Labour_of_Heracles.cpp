...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0;i < n;i++){
            nodes.push_back({a[i],deg[i]});
        }
        sort(nodes.rbegin(),nodes.rend());
        lli ans = accumulate(a.begin(), a.end(), 0LL);
        vector<lli> prefix(n-1,-1);
        prefix[0] = ans;
        lli idx = 0;
        for(lli i = 1;i < n-1 and idx < n;i++){
            if(nodes[idx].second < 2) {idx++;i--;continue;}
            nodes[idx].second--;
            prefix[i] = prefix[i-1] + nodes[idx].first;
        }

        for(lli i = 1;i < n-1;i++){
            if(prefix[i] == -1) prefix[i] = prefix[i-1];
        }
        for(lli i = 0;i < n-1;i++) cout <<prefix[i] << " ";
        cout << endl;
    }
    return 0;
}