...
int main() {
    ...
    while(t--) {
        ...
        vector<pair<lli,lli>> connections;
        vector<lli> powerGrids;
        while(!st.empty()){
            auto it = st.begin();
            lli idx = it -> second;
            lli currCost = it -> first;
            st.erase(it);
            ...
            ans += currCost;
            if(parent[idx]!= -1){
                connections.push_back({parent[idx]+1, idx+1});
            }else {
                powerGrids.push_back(idx + 1);
            }

            for(lli i = 0;i < n;i++){
                if(!vis[i]){
                    lli dist = abs(cities[idx].first - cities[i].first) + abs(cities[idx].second - cities[i].second);
                    lli newCost = dist * (k[idx] + k[i]);
                    if(newCost < cost[i]){
                        st.erase({cost[i], i});
                        cost[i] = newCost;
                        parent[i] = idx;
        ...
        cout << ans << endl;
        cout << powerGrids.size() << endl;
        for(auto &i : powerGrids) cout << i << " ";
        cout << endl;
