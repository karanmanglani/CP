...
int main() {
    ...
    while(t--) {
        ...
        lli s;cin >> s;
        vector<vector<lli>> adjj(n);
        for(lli i = 0; i < n; i++) {
            for(auto& p : adj[i]) {
                adjj[i].push_back(p.first);
            }
        }

        auto x = kosaraju(adjj);
        vector<vector<lli>> scc = get<0>(x);
        vector<vector<lli>> condensedGraph = get<1>(x);
        vector<lli> roots = get<2>(x);

        set<pair<lli,lli>> nsum;
        for(lli i = 0;i <= 1e5;i++){
            nsum.insert({(i*(i + 1)) / 2, i});
        }
        vector<lli> val(n,0);
        for(auto &comp : scc){
            lli root = comp[0];
            for(auto node : comp){
                for(auto x : adj[node]){
                    lli nd = x.first, w = x.second;
                    if(roots[nd] != roots[node]) continue;
                    lli k = nsum.lower_bound({w, 0})->second;
                    lli curr = k*w - (k-1)*k*(k+1)/6;
                    val[root] += curr;
                }
        ...
        vector<vector<pair<lli,lli>>> dag(n);
        for(lli i = 0;i < n;i++){
            for(auto child : adj[i]){
                lli nd = child.first, w = child.second;
                if(roots[i] != roots[nd]) dag[roots[i]].push_back({roots[nd], w});
            }
        }
        ...
        auto dfs = [&](lli node, auto&& self) -> void{
            ...
            for(auto child : dag[node]){
                lli nd = child.first, w = child.second;
                self(nd,self);
                dp[node] = max(dp[node], dp[nd] + w + val[node]);
            }
        };
