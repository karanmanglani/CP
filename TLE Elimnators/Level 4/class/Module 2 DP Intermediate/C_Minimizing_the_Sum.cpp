...
int main() {
    ...
    while(t--) {
        ...
        // dp[i][j] = min sum of i elements with at max j operations 
        vector<vector<lli>> dp(n, vector<lli> (k+1,LLONG_MAX));
        for(lli i = 0;i < n;i++){
            for(lli j = 0;j <= k;j++){
                if(i == 0) dp[i][j] = a[i];
                else{
                    lli ans = dp[i-1][j] + a[i];
                    lli mn = a[i];
                    for(lli z = 1;z <= j and i - z >= 0;z++){
                        mn = min(mn, a[i-z]);
                        if(i - z - 1 >= 0) ans = min(ans , dp[i-z-1][j-z] + mn * (z + 1));
                        else ans = min(ans , mn * (z + 1));
                    }
                    dp[i][j] = ans;
                }
            }
