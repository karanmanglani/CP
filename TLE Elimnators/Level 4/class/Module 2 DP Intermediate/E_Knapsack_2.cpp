...
int main() {
    ...
    while(t--) {
        ...
        // state : dp[i][j] => minimum weight needed to achive value j using first i items
        vector<vector<lli>> dp(n,vector<lli> (1e5+1,INT_MAX));
        for(lli i = 0;i < n;i++){
            for(lli j = 0;j <= 1e5;j++){
                if(j == 0) dp[i][j] = 0;
                else {
                    if(j >= a[i].second){
                        if(i == 0) {if(j == a[i].second) {dp[i][j] = a[i].first;}}
                        else dp[i][j] = min(dp[i-1][j],dp[i-1][j-a[i].second]+a[i].first);
                    } else {
