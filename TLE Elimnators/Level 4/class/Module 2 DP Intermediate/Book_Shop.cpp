...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : p) cin >> i;
        for(auto &i : pg) cin >> i;
        // state : dp[i][j] = maximumn no of pages that can be read with budget j if we take till books i into consideration
        // vector<vector<lli>> dp(n, vector<lli>(x+1,0));
        ...
        for(lli i = 1;i < n;i++){
            vector<lli> curr(x+1,0);
            for(lli j = 0;j <= x;j++){
                if(j == 0) {curr[j] = prev[j];continue;}
                if(j < p[i]){
                    curr[j] = prev[j];
                }else {
                    curr[j] = max(prev[j], prev[j-p[i]] + pg[i]);
                }
            }
            prev = curr;
        }
