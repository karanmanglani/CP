...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> cost(n,LLONG_MAX);
        for(lli i = 0;i < n;i++){
            cost[roots[i]] = min(cost[roots[i]], c[i]);
        }

        lli ans = 0;
        for(auto i : cost) if(i != LLONG_MAX) {
            ans += i;
        }

        lli ways = 1;
        for(auto i : scc) {
            lli minCost = LLONG_MAX;
            for(auto j : i) minCost = min(minCost, c[j]);
            lli cnt = 0;
            for(auto j : i) if(c[j] == minCost) cnt++;
            ways = (ways * cnt) % MOD;
        }
