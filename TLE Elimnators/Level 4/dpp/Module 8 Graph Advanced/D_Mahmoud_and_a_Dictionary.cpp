...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> dislike(n,-1);
        DisjointSet dsu(n); // Contains elements which means the same
        for(lli i = 0;i < m;i++){
            lli type;cin >> type;
            string a,b;cin >> a >> b;
            if(type == 1){
                lli u = dsu.fup(idx[a]), v = dsu.fup(idx[b]);
                if(dislike[u] == v || dislike[v] == u){
                    cout << "NO" << endl;
                }else if(u == v){
                    cout << "YES" << endl;
                }else{
                    if(dislike[u] == -1 && dislike[v] == -1){
                        dsu.ubs(u,v);
                    }else if(dislike[u] == -1 && dislike[v] != -1){
                        dsu.ubs(u,v);
                        dislike[dsu.fup(v)] = dislike[v];
                    }else if(dislike[u] != -1 && dislike[v] == -1){
                        dsu.ubs(u,v);
                        dislike[dsu.fup(u)] = dislike[u];
                    }else{
                        dsu.ubs(u,v);
                        dsu.ubs(dislike[u],dislike[v]);
                        dislike[dsu.fup(u)] = dsu.fup(dislike[u]);
                    }
                    cout << "YES" << endl;
                }
            }else{
                ...
                if(u != v){
                    if(dislike[u] == -1 && dislike[v] == -1){
                        ...
                        dsu.ubs(u,dislike[v]);
                        dsu.ubs(v,dislike[u]);
                        dislike[dsu.fup(u)] = dsu.fup(v);
                        dislike[dsu.fup(v)] = dsu.fup(u);
                    }
                    cout << "YES" << endl;
                }else{
                    cout << "NO" << endl;
        ...
        while(q--){
            ...
            if(u == v){
                cout << 1 << endl;
            }else if(dislike[u] == v || dislike[v] == u){
                cout << 2 << endl;
            }else {
                cout << 3 << endl;
            }
