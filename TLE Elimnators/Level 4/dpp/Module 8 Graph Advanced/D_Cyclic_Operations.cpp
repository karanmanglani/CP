...
int main() {
    ...
    while(t--) {
        ...
        auto scc = get<0>(kosaraju(adj));
        bool ans = true, fndk = false;
        int cnt = 0;
        ...
        for(auto &i : scc){
            if(i.size() == 1) cnt++;
                ...
                fndk = true;
                cycle[dsu.fup(i[0])] = true;
            }else ans = false;
        }
        for(lli i = 0;i < n;i++){
            if(!cycle[dsu.fup(i)]) ans = false;
        }
        cout << (ans && fndk? "YES" : "NO") << endl;
    }
    return 0;
}