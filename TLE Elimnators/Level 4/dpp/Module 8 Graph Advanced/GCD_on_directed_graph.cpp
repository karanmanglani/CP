...
tuple<vector<vector<lli>>,vector<set<lli>>,vector<lli>> kosaraju(vector<vector<lli>>& adj){
    lli n = adj.size();
    vector<lli> order;
    ...
    vector<set<lli>> condensedGraph(n);
    for(lli i = 0; i < n; i++) {
        for(lli child : adj[i]) {
            if(roots[i] != roots[child]) {
                condensedGraph[roots[i]].insert(roots[child]);
            }
        }
    }

    tuple<vector<vector<lli>>, vector<set<lli>>, vector<lli>> result;
...
int main() {
    ...
    while(t--) {
        lli n,m;cin >> n >> m;
        vector<lli> c(n);for(auto &i : c) cin >> i;
        ...
        auto x = kosaraju(adj);
        vector<vector<lli>> scc = get<0>(x);
        vector<set<lli>> condensedGraph = get<1>(x);
        vector<lli> roots = get<2>(x);
        set<lli> ur(roots.begin(), roots.end());
        vector<lli> uniqueRoots(ur.begin(), ur.end());
        map<lli,lli> idg;
        for(auto i : uniqueRoots){
            for(auto j : condensedGraph[i]) {
                idg[j]++;
            }
        }
        map<lli,lli> cost;
        ...
        while(!q.empty()){
            lli node = q.front();
            q.pop();
            for(auto child : condensedGraph[node]){
                idg[child]--;
                for(auto val : dp[node]){
                    dp[child].insert(__gcd(val,cost[child]));
                }
                if(idg[child] == 0){
                    q.push(child);
                }
