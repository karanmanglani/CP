...
int main() {
    ...
    while(t--) {
        ...
        auto x = kosaraju(adj);
        auto scc = get<0>(x);
        auto condensedGraph = get<1>(x);
        auto roots = get<2>(x);
        set<lli> uniqueRoots(roots.begin(), roots.end());
        vector<lli> ur(uniqueRoots.begin(), uniqueRoots.end());
        if(ur.size() == 1) {
            cout << "YES" << endl;
            return 0;
        }else{
            cout << "NO" << endl;
            vector<bool> vis(n,false);
            auto dfs = [&](lli from , lli to , auto&& self) -> bool{
                vis[from] = true;
                if(from == to) return true;
                for(lli child : adj[from]){
                    if(!vis[child]){
                        if(self(child, to, self)) {
                            return true;
                        }
                    }
                }
            ...
            for(lli i = 1;i < 2;i++){
                if(dfs(ur[i],ur[i-1],dfs)) {cout << ur[i - 1] + 1 << " " << ur[i] + 1 << endl;}
                else {cout << ur[i] + 1 << " " << ur[i - 1] + 1 << endl;}
            }
        }
    }
    return 0;
