...
int main() {
    ...
    while(t--) {
        lli n;cin >> n;
        vector<pair<lli,lli>> xSorted(n), ySorted(n);
        ...
        for(lli i = 1;i < n;i++){
            lli idx1 = idx[xSorted[i - 1]];
            lli idx2 = idx[xSorted[i]];
            edge[{idx1,idx2}] = min(abs(xSorted[i].first - xSorted[i - 1].first), abs(xSorted[i].second - xSorted[i - 1].second));
            edge[{idx2,idx1}] = min(abs(xSorted[i].first - xSorted[i - 1].first), abs(xSorted[i].second - xSorted[i - 1].second));
        }

        for(lli i = 1; i < n;i++){
            lli idx1 = idx[{ySorted[i - 1].second, ySorted[i - 1].first}];
            lli idx2 = idx[{ySorted[i].second, ySorted[i].first}];
            lli cost = min(abs(ySorted[i].first - ySorted[i - 1].first), abs(ySorted[i].second - ySorted[i - 1].second));
            
            auto key1 = make_pair(idx1, idx2);
            auto key2 = make_pair(idx2, idx1);

            if (edge.find(key1) != edge.end())
                edge[key1] = min(edge[key1], cost);
            else
                edge[key1] = cost;

            if (edge.find(key2) != edge.end())
                edge[key2] = min(edge[key2], cost);
            else
                edge[key2] = cost;
        }
        ...
        for(auto it : edge){
            lli u = it.first.first;
            lli v = it.first.second;
            lli cost = it.second;
