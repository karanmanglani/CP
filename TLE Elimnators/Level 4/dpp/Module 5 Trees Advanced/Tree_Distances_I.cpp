...
int main() {
    ...
    while(t--) {
        ...
        lli j = 0;
        for(lli k = 1;k < n;k++){
            if(dist[k] > dist[j]) j = k;
        }
        vector<lli> dist1(n,-1),dist2(n,-1);
        dist1[i] = 0;
        dist2[j] = 0;
        auto dfinder = [&](lli node, lli parent, vector<lli>& distance, auto &&self) -> void{
            for(lli child : adj[node]) {
                if(child != parent) {
                    distance[child] = distance[node] + 1;
                    self(child, node,distance, self);
                }
            }
        };

        dfinder(i, -1, dist1, dfinder);
        dfinder(j, -1, dist2, dfinder);
        for(lli i = 0;i < n;i++){
