...
int main() {
    ...
    while(t--) {
        ...
        height[0] = 0;
        auto dfs = [&](lli node, lli h, lli parent , auto&& self) -> void{
            height[node] = h;
            for(auto child : adj[node]) {
                if(child.first != parent && height[child.first] == -1) {
                    self(child.first, h + 1, node, self);
                }
            }
        };
        dfs(0, 0, -1, dfs);
        vector<vector<pair<lli,lli>>> parent(n,vector<pair<lli,lli>> (20,{-1,LLONG_MAX})); // parent,min edge weight in path to parent
        auto dfs2 = [&](lli node, lli par , auto &&self) -> void{
            if(par != -1){
                lli w = LLONG_MAX;
                if(adj2[node].count(par)) w = adj2[node][par];
                parent[node][0] = {par, w};
            }
            for(lli i = 1;i < 20;i++){
                if(parent[node][i-1].first != -1){
                    parent[node][i] = {parent[parent[node][i-1].first][i-1].first,min(parent[node][i-1].second, parent[parent[node][i-1].first][i-1].second)};
                }
            }
            for(auto child : adj[node]){
                if(child.first != par){
                    self(child.first, node, self);
                }
            }
        };
        ...
        while(q--){
            ...
            lli diff = height[a] - height[b];
            lli mn = LLONG_MAX;
            // case 1 : a and b are on the same path
            
            for(lli i = 0;i < 20;i++){
                if(diff & (1LL << i)){
                    mn = min(mn, parent[a][i].second);
                    a = parent[a][i].first;
                }
            }
            if(a == b){
                ...
                // a and b are not on the same path
                for(lli i = 19;i >= 0;i--){
                    if(parent[a][i].first != -1 && parent[b][i].first != -1 && parent[a][i].first != parent[b][i].first){
                        mn = min(mn, min(parent[a][i].second, parent[b][i].second));
                        a = parent[a][i].first;
                        b = parent[b][i].first;
                    } 
                }

                cout << min(mn, min(parent[a][0].second, parent[b][0].second)) << endl;
            }
        }
