...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> subtreeMax(n,0);
        vector<lli> dp(n,0);
        auto dfs = [&](lli node, lli parent, auto&& self) -> void{
            subtreeMax[node] = (a[node] == 1 ? 1 : -1);
            for(lli child : adj[node]){
                if(child != parent){
                    self(child, node, self);
                    if(subtreeMax[child] >= 0) {
                        subtreeMax[node] += subtreeMax[child];
                    }
                }
            }
        };
        dfs(0, -1, dfs);
        dp[0] = subtreeMax[0];
        auto fillTable = [&](lli node, lli parent , auto &&self) -> void{
            for(auto child : adj[node]){
                if(child != parent){
                    if(subtreeMax[child] >= 0) dp[child] = subtreeMax[child] + max(0LL,dp[node] - subtreeMax[child]);
                    else dp[child] = subtreeMax[child] + max(0LL,dp[node]);
                    self(child, node, self);
                }
            }
        };

        fillTable(0, -1, fillTable);
