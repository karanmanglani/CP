...
int main() {
    ...
    // lli t = 1;
    while(t--) {
        lli n,d12,d23,d13;cin >> n >> d12 >> d23 >> d13;
        vector<pair<lli,lli>> edges;
        lli mx = 0;

        if(d12 <= d13 && d12 + d23 == d13 && d13 <= n){
            lli curr = 4;
            lli parent = 1;
            while(d12 > 1){
                edges.push_back({parent, curr});
                mx = max({mx,curr,parent});
                d12--;
                parent = curr;
                curr++;
            }

            edges.push_back({parent, 2});
            parent = 2;
        ...
        }else if(d13 <= d12 && d13 + d23 == d12 && d12 <= n){
            lli curr = 4;
            lli parent = 1;
            while(d13 > 1){
                edges.push_back({parent, curr});
            ...
            while(curr <= n) {edges.push_back({1, curr});curr++;}
            ...
            if(d23 + x <= n){
                lli parent = 1, curr = 4;
                lli d2 = d12 - x, d3 = d13 - x;
                if(d2 + d3 != d23 || d2 <= 0 || d3 <= 0){
                    cout << "NO" << endl;
                    continue;
                } 
                while(x--){
