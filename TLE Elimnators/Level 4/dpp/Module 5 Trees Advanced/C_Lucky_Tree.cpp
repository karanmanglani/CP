...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> subTreeSize(n,0),luckyCnt(n,0);
        auto dfs = [&](lli node, lli parent , auto&& self) -> lli{
            subTreeSize[node] = 1;
            lli cnt = 0;
            for(auto edge : adj[node]){
                lli child = edge.first;
                lli weight = edge.second;
                if(child == parent) continue;
                self(child, node, self);
                subTreeSize[node] += subTreeSize[child];
                if(isLucky(weight)) cnt += subTreeSize[child];
                else cnt += luckyCnt[child];
            }
            return luckyCnt[node] = cnt;
        };

        vector<lli> dp(n,0);
        // dp[node] => number of elements to be considered for making triplets with node being the first element in the triplet
        dp[0] = dfs(0,-1,dfs);

        auto f = [&](lli node, lli parent , auto &&F) -> void{
            for(auto edge : adj[node]){
                lli child = edge.first, weight = edge.second;
                if(child == parent) continue;
                if(isLucky(weight)){
                    dp[child] = luckyCnt[child] + n - subTreeSize[child];
                }
                else{
                    dp[child] = dp[node];
                }
