...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> depth(n, 0);
        vector<vector<pair<lli,lli>>> parent(n,vector<pair<lli,lli>>(20,{-1,-1})); // Binary lifting with parent and weight sum 
        auto dfs = [&](lli node, lli par, lli d,auto &&self) -> void{
            depth[node] = d;
            for(lli i = 1;i < 20;i++){
                if(parent[node][i-1].first != -1){
                    lli p = parent[node][i-1].first;
                    lli w = parent[node][i-1].second;
                    if(p == -1) continue;
                    parent[node][i] = {parent[p][i-1].first, parent[p][i-1].second + w};
                }
            }
            for(auto child : adj[node]){
                if(child.first != par){
                    parent[child.first][0] = {node, child.second};
                    self(child.first, node, d+1, self);
                }
            }
        };
        ...
        auto findLCA = [&](lli u, lli v) -> lli{
            ...
            if(u == v) return u;
            for(lli i = 19;i >= 0;i--){
                if(parent[u][i].first != -1 && parent[v][i].first != -1 && parent[u][i].first != parent[v][i].first){
                    u = parent[u][i].first;
                    v = parent[v][i].first;
                } 
            }
        ...
        auto findDistance = [&](lli u, lli v) -> lli{
            lli lca = findLCA(u,v);
            lli diffu = depth[u] - depth[lca];
            lli diffv = depth[v] - depth[lca];
            lli sum = 0;
            for(lli i = 0;i < 20;i++){
                if((diffu >> i) & 1){
        ...
        while(cin >> str && str != "DONE"){
            if(str == "DIST"){
                ...
                u--; v--; k--;
                lli lca = findLCA(u,v);
                lli du = depth[u] - depth[lca];
                lli dv = depth[v] - depth[lca];
                if(k <= du){
                    lli node = u;
                    for(lli i = 0; i < 20; i++){
                        if((k >> i) & 1){
