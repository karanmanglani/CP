...
int main() {
    ...
    while(t--) {
        ...
        lli total = accumulate(a.begin(), a.end(), 0LL);
        vector<lli> dp(n, 0);
        vector<lli> subtreeSum(n,0);
        auto dfs = [&](lli node , lli parent,lli d, auto&& self) -> void{
            dp[0] += a[node] * d;
        ...
        auto fillTable = [&](lli node, lli parent , auto&& self) -> void{
            for(auto child : adj[node]){
                if(child != parent){
                    dp[child] = dp[node] + (total - 2 * subtreeSum[child]);
