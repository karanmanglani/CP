...
int main() {
    ...
    // lli t; cin >> t;
    lli t = 1;
    auto x = primeArr(200000);
    while(t--) {
        lli n;cin >> n;
        vector<lli> a(n);for(auto &i : a) cin >> i;
        auto spf = x.second.first;
        map<lli,lli> dp;
        lli ans = 0;
        // dp[prime] = length of logest subsequenc with this prime as a factor till now
        for(auto x : a){
            auto primeFactorisation = pfs(x,spf);
            lli curr = 1;
            for(auto &p : primeFactorisation){
                lli prime = p.first;
                curr = max(curr, dp[prime] + 1);
            }

            for(auto &p : primeFactorisation){
                lli prime = p.first;
                dp[prime] = max(dp[prime], curr);
            }
