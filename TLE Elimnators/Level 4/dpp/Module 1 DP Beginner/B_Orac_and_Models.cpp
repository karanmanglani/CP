...
int main() {
    ...
    while(t--) {
        ...
        lli maxi = 0;
        // state : dp[i] => Longest Good Subsequence ending at ith index
        vector<lli> dp(n,1);
        for(lli i = 0;i < n;i++){
            lli idx = i + 1;
            set<lli> st;
            for(lli j = 1;j * j <= idx;j++){
                if(idx % j == 0) {st.insert(j);st.insert(idx / j);}
            }
            vector<lli> pos;
            for(auto &j : st) if(j != idx and a[idx-1] > a[j-1]) pos.push_back(j);
            lli mx = 0;
            for(auto &j : pos) mx = max(mx,dp[j-1]);
            dp[i] += mx;
            maxi = max(maxi,dp[i]);
        }
        cout << maxi << endl;
