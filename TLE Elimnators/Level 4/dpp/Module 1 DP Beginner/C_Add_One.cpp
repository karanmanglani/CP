...
int main() {
    ...
    lli t; cin >> t;
    // lli t = 1;
    // precalculating for all of these
    // dp[i][j] = length of digit i after j operations
    vector<vector<lli>> dp(10,vector<lli>(2e5,0)); 
    for(lli i = 0;i < 10;i++){
        vector<lli> prev(10,0);
        prev[i] = 1;
        dp[i][0] = 1;
        lli ans = 1;
        for(lli j = 1;j <= 2e5;j++){
            vector<lli> curr(10,0);
            ans = (ans + prev[9]) % MOD;
            for(lli k = 9;k > 0;k--){
                curr[k] = prev[k-1];
            }
