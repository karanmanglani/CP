...
int main() {
    ...
    while(t--) {
        lli n;cin >> n;
        vector<ld> p(n);for(auto &i : p) cin >> i;
        // state : dp[i][j] => probability of getting heads in j coins if i coins are flipped
        ...
        ld ans = 0;
        for(lli i = n/2+1;i <= n;i++) ans += dp[n][i];
