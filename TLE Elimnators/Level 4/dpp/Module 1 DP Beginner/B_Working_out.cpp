...
int main() {
    ...
    while(t--) {
        ...
        vector<vector<lli>> dp1(n, vector<lli> (m,0)), dp2(n, vector<lli> (m,0)), dp3(n, vector<lli> (m,0)), dp4(n, vector<lli> (m,0));

        // dp1[i][j] = max sum from (0,0) to (i,j) using only right and down moves
        // dp2[i][j] = max sum from (n-1,m-1) to (i,j) using only left an up moves
        // dp3[i][j] = max sum from (n-1,0) to (i,j) using only right and up moves
        // dp4[i][j] = max sum form (0,m-1) to (i,j) using only left and down moves
        ...
        for(lli i = 0;i < n;i++){
            for(lli j = 0;j < m;j++){
                ...
                else dp1[i][j] = grid[i][j] + max(dp1[i-1][j], dp1[i][j-1]);
            }
        }
        ...
        for(lli i = n-1;i >= 0;i--){
            for(lli j = m-1;j >= 0;j--){
                if(i == n-1 && j == m-1) dp2[i][j] = grid[i][j];
                else if(i == n-1) dp2[i][j] = dp2[i][j+1] + grid[i][j];
                else if(j == m-1) dp2[i][j] = dp2[i+1][j] + grid[i][j];
                else dp2[i][j] = grid[i][j] + max(dp2[i+1][j], dp2[i][j+1]);
            }
        }
        ...
        for(lli i = n-1;i >= 0;i--){
            for(lli j = 0;j < m;j++){
                if(i == n-1 && j == 0) dp3[i][j] = grid[i][j];
                else if(i == n-1) dp3[i][j] = grid[i][j] + dp3[i][j-1];
                else if(j == 0) dp3[i][j] = grid[i][j] + dp3[i+1][j];
                else dp3[i][j] = grid[i][j] + max(dp3[i+1][j], dp3[i][j-1]);
            }
        }
        ...
        for(lli i = 0;i < n;i++){
            for(lli j = m-1;j >= 0;j--){
                if(i == 0 && j == m-1) dp4[i][j] = grid[i][j];
                else if(i == 0) dp4[i][j] = grid[i][j] + dp4[i][j+1];
                else if(j == m-1) dp4[i][j] = grid[i][j] + dp4[i-1][j];
                else dp4[i][j] = grid[i][j] + max(dp4[i-1][j], dp4[i][j+1]);
            }
        }
        ...
        for(lli i = 1;i+1 < n;i++){
            for(lli j = 1;j+1 < m ;j++){
                ans = max({ans,dp1[i][j-1] + dp2[i][j+1]
                 + dp3[i+1][j] + dp4[i-1][j],  dp1[i-1][j] + dp2[i+1][j]
                 + dp3[i][j-1] + dp4[i][j+1]});
            }
        }
