...
int main() {
    ...
    while(t--) {
        ...
        sort(c.begin(), c.end());
        // state : dp[i] = minimum coins to make i
        // transition : dp[i] = 1 + min(dp[i-c[j]]) for all j such that c[j] <= i
        vector<lli> dp(x + 1, -1);
        dp[0] = 0;
        for(lli i = 1;i <= x;i++){
            lli mn = LLONG_MAX;
            for(lli j = 0;j < n and c[j] <= i;j++){
                if(dp[i - c[j]] != -1) mn = min(mn , 1LL + dp[i - c[j]]);
            }
            dp[i] = (mn == LLONG_MAX) ? -1 : mn;
        }
        cout << dp[x] << endl;
    }
    return 0;
