...
int main() {
    ...
    while(t--) {
        ...
        lli ops;cin >> ops;
        bool goodStart = start == end;
        vector<lli> dpGood(ops+1,0), dpBad(ops+1,0);
        // dpGood[k] -> Number of ways to get end string from start in k operations 
        // dpBad[k] -> Number of ways to not get end string from start in k operations
        if(goodStart) {
            dpGood[0] = 1;
        } else {
            dpBad[0] = 1;
        }
        lli goodShifts = goodStart ? 1 : 0;
        for(lli i = 1;i < start.size();i++){
            string st = start.substr(i)  +  start.substr(0,i);
            if(st == end) goodShifts++;
        }
        lli badShifts = start.size() - goodShifts;
        for(lli i = 1;i <= ops;i++){
            dpGood[i] = (((dpGood[i-1] * (goodShifts-1)) % MOD + (dpBad[i-1] * goodShifts) % MOD)%MOD);
            dpBad[i] = (((dpBad[i-1] * (badShifts-1)) % MOD + (dpGood[i-1] * badShifts) % MOD)%MOD);
        }

        cout << dpGood[ops] << endl;
