...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : grid) for(auto &j : i) {cin >> j;if(j == 'G') isG = true;}
        if(!isG){
            cout << (grid[n-1][m-1] != 'B' ? "Yes" : "No") << endl;
            continue;
        }
        ...
        queue<pair<lli, lli>> bfs;
        if(grid[n-1][m-1] == 'G' || grid[n-1][m-1] == '.') bfs.push({n-1, m-1});
        lli cnt2 = 0;
        while(!bfs.empty()){
            auto f = bfs.front();
            bfs.pop();
