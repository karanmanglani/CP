...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> diceFace(n);
        auto f = [&](lli idx,vector<lli> diceFace, auto&& self) -> lli{
            if(idx == n) {
                map<pair<lli,lli>,bool> used;
                lli ans = 0;
                for(auto i : edges){
                    lli u = i.first, v = i.second;
                    if(!used[{diceFace[u],diceFace[v]}]){
                        ans++;
                        used[{diceFace[u],diceFace[v]}] = true;
                        used[{diceFace[v],diceFace[u]}] = true;
                    }
                }
                return ans;
            }
            lli ans = 0;
            for(lli i = 1;i <= 6;i++){
                vector<lli> temp = diceFace;
                temp[idx] = i;
                ans = max(ans, self(idx + 1, temp,self));
            }
            return ans;
        };

        cout << f(0,diceFace,f) << endl;
