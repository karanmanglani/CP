...
int main() {
    ...
    while(t--) {
        lli n,k;cin >> n >> k;
        vector<vector<lli>> arr(k,vector<lli> (n));
        for(auto &i : arr) for(auto &j : i) cin >> j;
        // Method 1: Constructive Algo TC -> O(n*k)
        // vector<lli> dis(n,0);
        // bool ans = true;
        // lli el = 0,cnt = 0;
        // for(lli i = 0;i < k;i++){
        ...
        //                 if(dis[curr - 1] == 0) {dis[curr - 1] = 50;el = curr;}
        //             }else{
        //                 curr = arr[i][j];
        //                 if(dis[curr - 1] == 0){
        //                     el = curr;
        //                     dis[curr - 1] = dis[arr[i][j-1] - 1] + 50;
        //                 }else{
        //                     if(dis[curr - 1] < dis[arr[i][j-1] - 1]) {
        //                         if(curr == el && !cnt){
        //                             cnt++;
        //                             dis[el - 1] = dis[arr[i][j-1] - 1] + 50;
        //                             i = 1;j = 0;
        //                             continue;
        //                         }
        //                         ans = false;
        ...
        // Method 2 : Using Topological Sort TC -> O(n*k)
        vector<vector<lli>> adj(n);
        vector<lli> idg(n,0);
        for(lli i = 0;i <k;i++){
            for(lli j = 2;j < n;j++){
                lli u = arr[i][j-1] - 1, v = arr[i][j] - 1;
                adj[u].push_back(v);
                idg[v]++;
            }
        }
