...
int main() {
    ...
    while(t--) {
        lli n,m,k;cin >> n >> m >> k;
        DisjointSet ds(n);
        vector<bool> gov(n+1, false),vis1(n+1, false);
        for(lli i = 0;i < k;i++) {
            lli x;cin >> x;
            gov[x] = true;
        }
        ...
        vector<bool> rootHasGov(n+1, false); 
        for(lli i = 1;i <= n;i++) rootHasGov[ds.fup(i)] = rootHasGov[ds.fup(i)] | gov[i];
        lli br = -1;
        for(lli i = 1;i <= n;i++){
            if(ds.fup(i) == i && rootHasGov[i]){
                if(br == -1 || ds.size[br] < ds.size[i]) br = i;
            }
        }

        for(lli i = 1; i <= n;i++){
            if(ds.fup(i) == i && !rootHasGov[i]){
                ds.size[br] += ds.size[i];
                ds.edges[br] += ds.edges[i];
            }
        }
