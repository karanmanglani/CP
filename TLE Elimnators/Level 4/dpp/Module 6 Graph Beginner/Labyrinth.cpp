...
int main() {
    ...
    while(t--) {
        ...
        vector<vector<lli>> dir(n,vector<lli> (m,-1));
        queue<tuple<lli,lli,lli>> q; // i,j,from where
        q.push({st.first,st.second,-1});
        vis[st.first][st.second] = true;
        while(!q.empty()){
            tuple<lli,lli,lli> curr = q.front();
            q.pop();
            lli i = get<0>(curr);
            lli j = get<1>(curr);
            lli from = get<2>(curr);
            dir[i][j] = from;
            if(i == ed.first && j == ed.second) break;
            vector<lli> dx = {-1, 0, 1, 0};
            vector<lli> dy = {0, 1, 0, -1};
            for(lli k = 0;k < 4;k++){
                lli ni = i + dx[k];
                lli nj = j + dy[k];
        ...
        if(dir[ed.first][ed.second] == -1) {
            cout << "NO" << endl;
            continue;
        }
        string path;
        lli i = ed.first, j = ed.second;
        vector<lli> dx = {-1, 0, 1, 0};
        vector<lli> dy = {0, 1, 0, -1};
        vector<char> moveChar = {'U', 'R', 'D', 'L'};
        while(dir[i][j] != -1) {
            lli d = dir[i][j];
            path += moveChar[d];
            i -= dx[d];
