...
int main() {
    ...
    while(t--) {
        lli n, m, steps;
        cin >> n >> m >> steps;
        vector<vector<lli>> a(n, vector<lli>(m - 1));
        vector<vector<lli>> b(n - 1, vector<lli>(m));
        for(lli i = 0; i < n; i++){
            for(lli j = 0; j < m - 1; j++){
                cin >> a[i][j];
            }
        ...
        auto valid = [](lli i, lli j, lli n, lli m) -> bool{
            return (i >= 0 && i < n && j >= 0 && j < m);
        };
        vector<vector<vector<lli>>> dp(n, vector<vector<lli>>(m, vector<lli>(21)));
        //state :  dp[i][j][k] => minimum possible sum of edges to go from (i, j) and come back to it in k steps        
        vector<lli> dx = {0, 0, 1, -1};
        vector<lli> dy = {1, -1, 0, 0};
        for(lli k = 0; k <= 20; k++){
            for(lli i = 0; i < n; i++){
                for(lli j = 0; j < m; j++){
                    if(k & 1){ 
                        ...
                        if(k == 0){
                            ...
                            dp[i][j][k] = 2e9;
                            for(lli cases = 0; cases < 4; cases++){
                                lli newI = i + dx[cases];
                                lli newJ = j + dy[cases];
                                if(valid(newI, newJ, n, m)){ 
                                    lli edgeWeight = 0;
                                    if(newI != i){
                                        edgeWeight = b[min(newI, i)][j];
                                    }else{
                                        edgeWeight = a[i][min(newJ, j)];
                                    }
