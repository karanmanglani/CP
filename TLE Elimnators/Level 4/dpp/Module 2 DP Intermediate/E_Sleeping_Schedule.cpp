...
int main() {
    ...
    lli t = 1;
    while(t--) {
        lli n,h,l,r;cin >> n >> h >> l >> r;
        vector<lli> a(n);for(auto &i : a) cin >> i;
        // state : dp[i][j] = till i-th index , if current sleeping time is j then maximum no of good sleep hours achieved
        vector<vector<lli>> dp(n + 1, vector<lli>(h, LLONG_MIN));
        dp[0][0] = 0;
        for(lli i = 1;i <= n;i++){
            for(lli j = 0;j < h;j++){
                lli el = a[i-1];
                dp[i][j] = max(dp[i-1][(j - el + h)%h], dp[i-1][(j - el + 1 + h)%h]);
                if(j >= l && j <= r && dp[i][j] != LLONG_MIN) dp[i][j]++;
            }
        }
