...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : a) cin >> i;
        for(auto &i : b) cin >> i;
        // state : dp[i][j] => number of sequences of length i with last element is j such that a[i] <= c[i] <= j

        vector<vector<lli>> dp(n, vector<lli> (3001,0));
        vector<lli> pre(3001,0);
        for(lli i = 0;i < n;i++){
            vector<lli> curr(3001,0);
            for(lli j = 0;j < 3001;j++){
                if(i == 0){
                    if(a[i] <= j && j <= b[i]){
                        dp[i][j] = 1;
                    }
                    curr[j]  = (j == 0) ? dp[i][j] : (dp[i][j] + curr[j - 1]) % MOD;
                }else{
                    if(a[i] <= j and j <= b[i]){
                        dp[i][j] = pre[j];
                    }
