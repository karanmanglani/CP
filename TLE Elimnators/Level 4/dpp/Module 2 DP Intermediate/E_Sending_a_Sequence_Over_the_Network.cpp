...
int main() {
    ...
    while(t--) {
        ...
        // state : dp[i] => till i it is possible to create the sequence
        vector<bool> dp(n+1,false);
        dp[0] = true;
        for(lli i = 1;i <= n;i++){
            // if this is length to the left of segment
            if(i + a[i-1] <= n && dp[i-1] == true) dp[i + a[i-1]] = true;
            // if this is length to the right of segment
            if(i - a[i-1] - 1 >= 0 and dp[i-a[i-1]-1] == true) dp[i] = true;
        }
