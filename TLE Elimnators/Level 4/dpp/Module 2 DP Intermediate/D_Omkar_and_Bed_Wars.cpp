...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0;i < 2;i++){
            for(lli j = 0;j < 2;j++){
                vector<vector<vector<lli>>> dp(n, vector<vector<lli>>(2, vector<lli> (2,-1)));
                auto f = [&](lli i, lli l , lli sl , string &s, lli fir, lli sec, auto &&F){
                    if(i == n-1){
                        lli anss = 2e5+1;
                        // taking l
                        if( (l != 0 || sl != 0) && (fir != 0 || l != 0) && (fir != 0 || sec != 0)){
                            anss = min(anss , (s[i] == 'L' ? 0LL : 1LL));
                        }
                        // taking r
                        if( (l != 1 || sl != 1) && (fir != 1 || l != 1) && (fir != 1 || sec != 1)){
                            anss = min(anss , (s[i] != 'R' ? 1LL : 0LL));
                        }
                        return anss;
                    }else{
                        ...
                        // taking l
                        if( (l != 0 || sl != 0)){
                            anss = min(anss , (s[i] != 'L' ? 1LL : 0LL) + F(i+1,0,l,s,fir,sec,F));
                        }
                        // taking r
                        if( (l != 1 || sl != 1)){
                            anss = min(anss , (s[i] != 'R' ? 1LL : 0LL) + F(i+1,1,l,s,fir,sec,F));
                        }
                        return dp[i][l][sl] = anss;
                    }
                };
                lli ct1 = 0 , ct2 = 0;
                
                if(s[0] == 'L')ct1 = (i == 0 ? 0 : 1);
                else ct1 = (i == 0 ? 1 : 0);
                
                if(s[1] == 'L') ct2 = (j == 0 ? 0 : 1);
                else ct2 = (j == 0 ? 1 : 0);
                
                ans = min(ans , ct1 + ct2 + f(2,j,i,s,i,j,f));
            }
        }
        cout << ans << endl;
    }
