...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : b) cin >> i;
        for(auto &i : c) cin >> i;
        // state : dp[i][j] => maximum coins you can get till index i with j operations left
        k = min(k , 12 * n);
        vector<vector<lli>> dp(n, vector<lli> (k+1,0));
        vector<lli> opsNeeded(1001,LLONG_MAX);
        opsNeeded[1] = 0;
        for(lli i = 1;i <= 1000 ;i++){
            lli x = 1;
            while(i/x > 0){
                if(i + (i / x) <= 1000 and opsNeeded[i + (i / x)] > opsNeeded[i] + 1){
                    opsNeeded[i + (i / x)] = opsNeeded[i] + 1;
                }
                x++;
            }
        }
        for(lli i = 0;i < n; i++){
            for(lli j = 0;j <= k;j++){
                ...
                if(i == 0){
                    ...
                    dp[i][j] = dp[i-1][j];
                    if(opsNeeded[b[i]] <= j){
                        dp[i][j] = max(dp[i-1][j], dp[i-1][j-opsNeeded[b[i]]] + c[i]);
                    }
                }
            }
        }
