...
int main() {
    ...
    while(t--) {
        lli m,x;cin >> m >> x;
        vector<lli> c(m), h(m);
        for(lli i = 0;i < m;i++) cin >> c[i] >> h[i];
        lli maxH = accumulate(h.begin(), h.end(), 0LL);


        vector<vector<lli>> dp(m+1 , vector<lli> (maxH + 1, LLONG_MAX));
        dp[0][0] = 0;
        if(c[0] == 0) dp[0][h[0]] = 0;
        for(lli idx = 1;idx < m;idx++){
            dp[idx][0] = 0;
            for(lli hp = 1;hp <= maxH;hp++){
                
                // Not Take
                if(dp[idx-1][hp] != LLONG_MAX){
                    dp[idx][hp] = min(dp[idx-1][hp], dp[idx][hp]);
                }

                // Take
                if(hp - h[idx] >= 0 && dp[idx-1][hp - h[idx]] != LLONG_MAX){
                    if(dp[idx-1][hp - h[idx]] + c[idx] <= x*(idx)){
                        dp[idx][hp] = min(dp[idx-1][hp - h[idx]] + c[idx],dp[idx][hp]);
                    }
                }
