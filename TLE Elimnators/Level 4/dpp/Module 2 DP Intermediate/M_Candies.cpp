...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> a(n);
        for(auto &i : a) cin >>i;
        // state : dp[i][j] => number of ways to distribute j candies to i children
        vector<vector<lli>> dp(n, vector<lli>(k + 1, 0));
        vector<lli> pre(k+1,0);
        for(lli i = 0;i < n;i++){
            vector<lli> curr(k+1,0);
            for(lli j = 0;j <= k;j++){
                if(i == 0) {
                    if(j <= a[i]) dp[i][j] = 1;
                }else{
                    dp[i][j] = (j - min(j,a[i]) - 1 >= 0) ? (pre[j] - pre[j - min(j,a[i]) - 1] + MOD) % MOD : pre[j]; 
                }
                curr[j] = (j == 0) ? dp[i][j] : (curr[j-1] + dp[i][j]) % MOD;
            }
            pre = curr;
        }
        cout << dp[n-1][k] << endl;
