...
int main() {
    ...
    while(t--) {
        string l, r;cin >> l >> r;
        lli n = r.length();
        vector<vector<vector<lli>>> dp(n, vector<vector<lli>>(2, vector<lli> (4,-1)));
        // dp[idx][tight][nonZero]
        auto f = [&](lli idx, lli tight , lli nonZero, auto &&F) -> lli{
            if(idx == n){
                return (nonZero <= 3) ? 1 : 0;
            }
            if(nonZero > 3) return 0;
            if(dp[idx][tight][nonZero] != -1) return dp[idx][tight][nonZero];
            lli limit = (tight) ? (r[idx] - '0') : 9;
            dp[idx][tight][nonZero] = 0;
            for(lli i = 0; i <= limit; i++){
                lli newTight = tight && (i == limit);
                lli newNonZero = nonZero + (i > 0);
                dp[idx][tight][nonZero] += F(idx + 1, newTight, newNonZero, F);
            }
        ...
        vector<vector<vector<lli>>> dp2(n, vector<vector<lli>>(2, vector<lli> (4,-1)));
        n = l.length();
        auto f2 = [&](lli idx, lli tight , lli nonZero, auto &&F) -> lli{
            if(idx == n){
                return (nonZero <= 3) ? 1 : 0;
            }
            if(nonZero > 3) return 0;
            if(dp2[idx][tight][nonZero] != -1) return dp2[idx][tight][nonZero];
            lli limit = (tight) ? (l[idx] - '0') : 9;
            dp2[idx][tight][nonZero] = 0;
            for(lli i = 0; i <= limit; i++){
                lli newTight = tight && (i == limit);
                lli newNonZero = nonZero + (i > 0);
                dp2[idx][tight][nonZero] += F(idx + 1, newTight, newNonZero, F);
            }
            return dp2[idx][tight][nonZero];
        };
        ans -= f2(0,1,0,f2);
        lli cnt = 0;
        for(auto i : l) if(i != '0') cnt++;
        if(cnt <= 3) ans++;
        cout << ans << endl; 
    }
    return 0;
