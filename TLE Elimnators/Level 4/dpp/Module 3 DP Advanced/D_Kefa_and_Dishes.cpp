...
int main() {
    ...
    while(t--) {
        ...
        auto f = [&](lli last, lli mask, auto &&self) -> lli{
            lli dishes = __builtin_popcount(mask);
            if(dishes == m) return 0;
            if(last == -1){
                lli ans = 0;
                for(lli i = 0;i < n;i++){
                    if(mask & (1 << i)) continue;
                    ans = max(ans, self(i, mask | (1 << i), self) + a[i]);
                }
                return ans;
            }
            ...
            for(lli i = 0;i < n;i++){
                if(mask & (1 << i)) continue;
                lli extra = mp.count({last,i}) ? mp[{last,i}] : 0;
                dp[last][mask] = max(dp[last][mask], self(i, mask | (1 << i), self) + a[i] + extra);
            }
            return dp[last][mask];
        };
