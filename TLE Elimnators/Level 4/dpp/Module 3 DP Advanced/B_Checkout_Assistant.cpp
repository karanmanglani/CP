...
int main() {
    ...
    while(t--) {
        lli n;cin >> n;
        vector<pair<lli,lli>> a(n);for(auto &i : a) cin >> i.first >> i.second;
        vector<vector<lli>> dp(n, vector<lli>(4001,-1));
        auto f = [&](lli i, lli j, auto&& self) -> lli{
            if(i == n) {
                if(j < 2000) return 1e15;
                else return 0;
            }
            ...
            // steal this item
            dp[i][j] = min(dp[i][j], self(i+1, j-1,self));
            // purchase this item
            dp[i][j] = min(dp[i][j], self(i+1,min(4000LL , j + a[i].first), self) + a[i].second);
            return dp[i][j];
        };
        cout << f(0,2000,f) << endl;
    }
    return 0;
}