...
int main() {
    ...
    while(t--) {
        lli n,k;cin >> n >> k;
        vector<lli> c(n*k); for(auto &i : c) cin >> i;
        vector<lli> f(n); for(auto &i : f) cin >> i;
        vector<lli> h(k+1);
        h[0] = 0;
        for(lli i=1;i <= k;i++) cin >> h[i];

        // dp[i][j] => Maximum Happiness achieveable if we distrubute cards among i-people having same favourite number and j-cards in the deck having that favourite number

        vector<vector<lli>> dp(n+1,vector<lli> (n*k + 1,0));
        for(lli i = 1;i <= n;i++){
            for(lli j = 0;j <= i*k;j++){
                for(lli t = 0;t <= k;t++){
                    if(j-t >= 0){
        ...
        for(auto i : favCnt){
            lli num = i.first;
            ans += dp[i.second][min(crdCnt[num], i.second * k)];
        }
