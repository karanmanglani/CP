...
int main() {
    ...
    // your code goes here
    vector<vector<vector<lli>>> dp(31, vector<vector<lli>> (31, vector<lli> (51,-1)));
    // dp[n][m][k] => min cost to get area k  from rectangle of dimensions n*m

    auto f = [&](lli n , lli m , lli k , auto &&F) -> lli{
        if(n*m < k || k < 0 ) return LLONG_MAX;
        ...
        for(lli i = 1;i < n;i++){
            ...
            lli a1 = F(i,m,k,F);
            lli a2 = F(n-i,m,k,F);
            lli a3 = F(i,m,k - (n-i)*m,F);
            lli a4 = F(n-i,m,k - i*m,F);

            if(a1 == LLONG_MAX && a2 == LLONG_MAX && a3 == LLONG_MAX && a4 == LLONG_MAX){
                continue;
            }

            dp[n][m][k] = min(dp[n][m][k],cost + min({a1,a2,a3,a4}));
        }
        ...
        for(lli j = 1;j < m;j++){
            ...
            lli a1 = F(n,j,k,F);
            lli a2 = F(n,m-j,k,F);
            lli a3 = F(n,j,k - (m-j)*n,F);
            lli a4 = F(n,m-j,k - j*n,F);
