...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> dp(n+1, -1);
        auto x = pcf(2*n);
        vector<lli> fct = x.first, ifct = x.second;
        auto f = [&](lli idx, auto &&self) -> lli {
            if(idx == n) return 1LL;
            if(dp[idx] != -1) return dp[idx];
            dp[idx] = 0;
            if(a[idx] <= 0) return dp[idx] = 0;
            if(idx + a[idx] + 1 > n) return dp[idx] = 0;
            for(lli i = idx + a[idx] + 1; i <= n; i++){
                dp[idx] = (dp[idx] + (ncrf(i - idx - 1, a[idx],fct,ifct) * self(i, self)) % MOD) % MOD;
            }
            return dp[idx];
        };
        lli ans = 0;
        for(lli i = 0; i < n; i++){
            if(a[i] > 0) ans = (ans + f(i, f)) % MOD;
        }
        cout << ans << endl;
    }
    return 0;
