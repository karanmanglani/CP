...
int main() {
    ...
    while(t--) {
        ...
        vector<vector<vector<lli>>> dp(n-m,vector<vector<lli>> (2001,vector<lli> (2,-1)));
        // dp[idx][sc][qStarted] => Number of ways to reach balanced state if we at idx with score sc and q part started or not

        auto f = [&](lli idx, lli sc, lli qStarted, auto &&F) -> lli{
            if(idx == n-m){
                if(qStarted == 0 && sc < abs(mn)){
                    return 0;
                }

                if(qStarted == 0) return (sc+score == 0);

                return (sc == 0);
            }
            if(sc < 0) return 0;
            if(dp[idx][sc][qStarted] != -1) return dp[idx][sc][qStarted];

            dp[idx][sc][qStarted] = 0;

            // Try to place '('
            dp[idx][sc][qStarted] = (dp[idx][sc][qStarted] + F(idx + 1, sc + 1, qStarted,F)) % MOD;

            // Try to place ')'
            dp[idx][sc][qStarted] = (dp[idx][sc][qStarted] + F(idx + 1, sc - 1, qStarted,F)) % MOD;

            // Try to move to q
            if(!qStarted && sc >= abs(mn)){
                dp[idx][sc][qStarted] = (dp[idx][sc][qStarted] + F(idx, sc + score, 1,F)) % MOD;
            }

            return dp[idx][sc][qStarted];
        };
