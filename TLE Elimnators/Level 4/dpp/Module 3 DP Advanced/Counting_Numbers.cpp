...
int main() {
    ...
    while(t--) {
        string a,b;cin >> a >> b;
        auto help = [&](string num) -> lli{
            lli n = num.length();
            vector<vector<vector<vector<lli>>>> dp(n, vector<vector<vector<lli>>>(2,vector<vector<lli>>(10,vector<lli>(2,-1))));
            auto f = [&](lli idx, lli tight , lli prev,lli lz ,auto &&self) -> lli{
                if(idx == n) return 1;
                if(dp[idx][tight][prev][lz] != -1) return dp[idx][tight][prev][lz];
                lli limit = (tight ? num[idx] - '0' : 9);
                dp[idx][tight][prev][lz] = 0;
                for(lli i = 0;i <= limit;i++){
                    if(idx == 0 or prev != i or (i == 0 and lz == 1)){
                        dp[idx][tight][prev][lz] += self(idx + 1, tight & (i == limit),i,(lz & (i == 0)),self);
                    }
                }
