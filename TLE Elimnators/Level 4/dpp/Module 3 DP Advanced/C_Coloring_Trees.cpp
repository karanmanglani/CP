...
int main() {
    ...
    while(t--) {
        ...
        // state dp[i][j][p] = minimum cost  i-th element with previous colour being the  j-th color and p partitions going from index i to n-1
        auto f = [&](lli idx, lli col, lli partitions, auto&& self) -> lli{
            if(partitions > k) return LLONG_MAX;
            if(idx == n and partitions == k) return 0;
            if(idx == n) return LLONG_MAX;
            if(dp[idx][col][partitions] != -1) return dp[idx][col][partitions];
            
            lli ans = LLONG_MAX;
            if(c[idx] == 0){
                for(lli i = 1;i <= m;i++){
                    lli ncst = self(idx + 1, i , (col == i ? partitions : partitions + 1), self);
                    if(ncst == LLONG_MAX) continue;
                    ans = min(ans , cost[idx][i-1] + ncst);
                }
            }else {
                lli ncst = self(idx + 1, c[idx], (col == c[idx] ? partitions : partitions + 1), self);
                if(ncst == LLONG_MAX) return LLONG_MAX;
                ans = min(ans , ncst);
            }

            return dp[idx][col][partitions] = ans;
        };
