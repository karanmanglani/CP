...
int main(){
    ...
    for(auto R : rGroups){
        vector<vector<lli>> dp(1 << n, vector<lli>(n + 1, -1));

        auto f = [&](lli mask, auto &&F) -> vector<lli> {
            ...
            vector<lli> cnt(n + 1, 0);
            lli timeUsed = 0, score = 0;
            for (lli i = 0; i < n; i++){
                if(mask & (1 << i)){
                    timeUsed += (R >= d[i]) ? ts[i] : tf[i];
                    if(R >= d[i]) score++;
                }
            }
            
            if(timeUsed >= t){
                lli rem = n - __builtin_popcount(mask);
                cnt[score] = (cnt[score] + fact[rem]) % MOD;
                return dp[mask] = cnt;
            }

            lli k = n - __builtin_popcount(mask);
            for(int i = 0; i < n; i++){
                if(!(mask & (1 << i))){
                    lli cost = (R >= d[i]) ? ts[i] : tf[i];
                    if(timeUsed + cost > t){
                        if (k > 0) {
                            cnt[score] = (cnt[score] + fact[k - 1]) % MOD;
                        }
                    } else {
                        vector<lli> temp = F((mask | (1 << i)), F);
                        for(int j = 0; j <= n; j++){
        ...
        vector<lli> counts = f(0, f);
        lli num = 0, den = 0;
        for(lli i = 0; i <= n; i++){
            num = (num + (i * counts[i]) % MOD) % MOD;
            den = (den + counts[i]) % MOD;
        }

        if (den == 0) ans[R] = 0;
        else ans[R] = (num * mi(den)) % MOD;
    }

    vector<lli> rVec(rGroups.begin(), rGroups.end());
    for(auto curR : r){
        auto it = upper_bound(rVec.begin(), rVec.end(), curR);
        lli keyR = 1;
        if(it != rVec.begin()){
            keyR = *(--it);
        }
