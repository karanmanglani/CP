...
int main() {
    ...
    while(t--) {
        ...
        vector<bool> vis(n,false);
        lli rem = 0;
        while(k-- and (mp[1].size() > 0 || mp[0].size() > 0) ){
            queue<lli> q;

            for(auto it : mp[0]) {q.push(it);vis[it] = true;}
            ...
            while(!q.empty()){
                ...
                q.pop();
                for(lli child : adj[node]){
                    if(mp[idg[child]].find(child) != mp[idg[child]].end()){
                        mp[idg[child]].erase(child);
                    }
                    idg[child]--;
                    if(idg[child] >= 0 && !vis[child]){
                        mp[idg[child]].insert(child);
                    }
                }
            }
        }
