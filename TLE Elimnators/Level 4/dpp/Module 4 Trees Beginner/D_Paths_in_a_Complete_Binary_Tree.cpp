...
int main() {
    ...
    while(t--) {
        ...
        lli mxd = 0;
        while((1LL << mxd) != n+1) mxd++;
        auto depth = [&](lli node) -> lli{
            lli d = 0, curr = (n+1)/2;
            while(curr != node){
                lli delta = be(2,mxd - d - 2);
                d++;
                if(curr > node){
                    curr -= delta;
                } else {
        ...
        while(q--){
            ...
            for(auto i : s){
                if(i == 'L'){
                    lli delta = be(2,mxd - d - 2);
                    if(delta < curr && depth(curr - delta) == (d+1)) {curr -= delta;d++;}
                }else if(i == 'R'){
                    lli delta = be(2,mxd - d - 2);
                    if(curr + delta <= n && depth(curr + delta) == (d+1)) {curr += delta;d++;}
                }else{
                    lli delta = be(2,mxd - d - 1);
                    lli left = curr - delta;
                    lli right = curr + delta;

                    if(left > 0 && depth(left) == (d-1)){
                        curr = left;
                        d--;
                    }else if(right <= n && depth(right) == (d-1)){
                        curr = right;
                        d--;
                    }
                }
