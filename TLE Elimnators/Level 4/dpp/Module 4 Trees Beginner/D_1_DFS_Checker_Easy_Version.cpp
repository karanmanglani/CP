...
int main(){
    ...
    while(t--){
        ...
        vector<int> tin(n), tout(n), par(n,-1);
        int timer=0;
        ...
        auto checkPair = [&](int i)->bool{
            if(i<0||i+1>=n) return true;
            return isAnc(par[p[i+1]],p[i]);
        };
        int bad = (p[0]!=0);
        for(int i=0;i+1<n;i++) if(!checkPair(i)) bad++;
        while(q--){
            int x,y; cin >> x >> y; x--; y--;
            vector<int> idxs;
            for(int d=-1;d<=0;d++){
                if(x+d>=0&&x+d<n-1) idxs.push_back(x+d);
                if(y+d>=0&&y+d<n-1) idxs.push_back(y+d);
            }
