...
int main() {
    ...
    while(t--) {
        ...
        set<lli> ans;
        auto dfs = [&](lli node, lli parent , lli curr, auto&& self) -> void{
            for(auto child : adj[node]){
                if(child.first == parent) continue;
                if(child.second == 2){
                    if(ans.find(curr) != ans.end()) ans.erase(curr);
                    ans.insert(child.first);
                    self(child.first, node, child.first,self);
                }else {
                    self(child.first , node, curr, self);
                }
            }
        };
