...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> col(n);for(auto &i :col) cin >> i;
        lli root = 0,sroot = 0;
        auto dfs1 = [&](lli node, lli par, auto&& self) -> void{
        ...
        dfs1(0, -1, dfs1);
        vector<bool> anss(n,false);
        auto dfs2 = [&](lli node, lli par , lli c,auto&& self) -> bool{
            bool ans = true;
            if(col[node] != c and c != -1) ans = false;
            for(auto child : adj[node]){
                if(child == par) continue;
                if(c == -1){
                    ans &= self(child, node, col[child], self);
                }else {
                    ans &= self(child, node, c, self);
                }
            }
            return anss[node] = ans;
        };


        bool ans = dfs2(root, -1, -1, dfs2);
        
        if(ans) {
            cout << "YES" << endl;
            cout << root + 1 << endl; 
        } else {
            ans = dfs2(sroot, -1, -1, dfs2);
            if(ans) {
                cout << "YES" << endl;
                cout << sroot + 1 << endl; 
            } else {
