...
int main() {
    ...
    lli u = max_element(dist1.begin(), dist1.end()) - dist1.begin();

    vector<lli> dist2(n, -1);
    q = queue<lli>();
    dist2[u] = 0;
    q.push(u);
    while(!q.empty()) {
    ...
    lli v = max_element(dist2.begin(), dist2.end()) - dist2.begin();
    lli d = dist2[v];

    vector<lli> dist3(n, -1);
    q = queue<lli>();
    dist3[v] = 0;
    q.push(v);
    while(!q.empty()) {
        ...
        for(lli child : adj[node]) {
            if(dist3[child] == -1) {
                dist3[child] = dist3[node] + 1;
                q.push(child);
            }
        }
    }

    vector<lli> endpoints;
    for(lli i = 0; i < n; i++) {
        if(dist2[i] == d || dist3[i] == d) {
            endpoints.push_back(i + 1); 
        }
    }

    vector<bool> ep(n,false);
    for(lli i = 0; i < endpoints.size(); i++) {
        ep[endpoints[i] - 1] = true;
    }
    for(lli i = 0;i < n;i++){
        cout << (ep[i] ? d + 1 : d) << endl;
    }
