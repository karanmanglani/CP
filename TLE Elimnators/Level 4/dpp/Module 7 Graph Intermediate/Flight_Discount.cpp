...
int main() {
    ...
    while(t--) {
        ...
        vector<vector<pair<lli,lli>>> adjR(n);
        for(lli i = 0;i < m;i++){
            lli u,v;cin >> u >> v;
            u--; v--;
            lli w;cin >> w;
        ...
        auto dijkstra = [&](lli src,vector<vector<pair<lli,lli>>>& graph) -> vector<lli> {
        ...
        vector<lli> a = dijkstra(0,adj), b= dijkstra(n - 1,adjR);
        lli ans = LLONG_MAX;
        for(lli i = 0;i < n;i++){
            for(auto &child : adj[i]){
                lli x = child.first;
                lli w = child.second;
                if(a[i] != LLONG_MAX && b[x] != LLONG_MAX) ans = min(ans, (w/2) + a[i] + b[x]);
            }
        }
        cout << ans << endl;
    }
