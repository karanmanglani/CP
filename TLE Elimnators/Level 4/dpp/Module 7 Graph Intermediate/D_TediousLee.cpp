...
int main() {
    ...
    // lli t = 1;
    bool flag = true;
    vector<vector<lli>> dp(2e6+1,vector<lli> (3,0));
    dp[1][0] = 1;
    for(lli i = 2;i <= 2e6;i++){
        // Counting number of vertices with zero children
        dp[i][0] = (dp[i-1][0] + 2*dp[i-1][1]) % MOD;

        // Counting number of verices with one children
        dp[i][1] = dp[i-1][0];

        // Counting number of claws added in this step
        dp[i][2] = dp[i-1][1];
    }

    lli modZero = 0, modOne = 0, modTwo = 0;
    vector<lli> ans(2e6+1,0); 
    for(lli i = 1;i <= 2e6;i++){
        if(i % 3 == 0){
            modZero = (modZero + dp[i][2]) % MOD;
            ans[i] = (modZero * 4) % MOD;
        }else if(i % 3 == 1){
            modOne = (modOne + dp[i][2]) % MOD;
            ans[i] = (modOne * 4) % MOD;
        }else{
            modTwo = (modTwo + dp[i][2]) % MOD;
            ans[i] = (modTwo * 4) % MOD;
        }
