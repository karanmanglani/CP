...
int main() {
    ...
    while(t--) {
        ...
        queue<tuple<lli,lli,pair<lli,lli>>> q;
        map<pair<lli,lli>, pair<lli,lli>> ans;
        lli dx[] = {1, 0, -1, 0};
        lli dy[] = {0, 1, 0, -1};
        for(lli i=0;i < n;i++){
            lli x,y;cin >> x >> y;
        ...
        for(auto point : points){
            for(lli i=0;i < 4;i++){
                lli nx = point.first + dx[i];
                lli ny = point.second + dy[i];
                if(mp.find({nx,ny}) == mp.end()){
                    ans[{point.first, point.second}] = {nx, ny};
                    q.push({point.first, point.second,{nx,ny}});
                }
            }
        }
        ...
        for(auto point : points){
            cout << ans[point].first << " " << ans[point].second << endl;
        }
    }
    return 0;
}