...
int main() {
    ...
    while(t--) {
        lli n;cin >> n;
        DisjointSet dsu(n);
        vector<pair<lli,lli>> wasteEdges;
        for(lli i = 0;i < n-1;i++){
            lli u,v;cin >> u >> v;
            if(dsu.fup(u) != dsu.fup(v)){
                dsu.ubs(u,v);
            }else{
                wasteEdges.push_back({u,v});
            }
        }

        vector<pair<lli,lli>> wantedEdges;
        vector<lli> roots;
        for(lli i = 1;i <= n;i++){
            if(dsu.fup(i) == i) roots.push_back(i);
        }

        for(lli i = 1;i < roots.size();i++){
            wantedEdges.push_back({roots[i-1],roots[i]});
        }

        cout << wantedEdges.size() << endl;
        for(lli i = 0;i < wantedEdges.size();i++){
            cout << wasteEdges[i].first << " " << wasteEdges[i].second << " " << wantedEdges[i].first << " " << wantedEdges[i].second << endl;
        }
    }
    return 0;
}