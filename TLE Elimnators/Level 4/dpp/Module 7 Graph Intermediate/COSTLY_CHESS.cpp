    ...
    while(t--) {
        ...
        while(cin >> a >> b >> c >> d){
            ...
            while(!pq.empty()){
                ...
                if(dist.find({a,b}) != dist.end() && dist[{a,b}] < dst) continue;
                if(a == c && b == d){
                    cout << dst << endl; 
                    fnd = true;
                    break;
                ...
                // 2 right 1 up
                lli na = a-1, nb = b+2;
                if(na >= 0 && nb >= 0 && na <= 7 && nb <= 7){
                    if(dist.find({na,nb}) == dist.end() || dist[{na,nb}] > (dst + (na * a) + (nb * b))){
                        dist[{na,nb}] = dst + (na * a) + (nb * b);
                        pq.push({dst + (na * a) + (nb * b),{na,nb}});
                    }
                }
