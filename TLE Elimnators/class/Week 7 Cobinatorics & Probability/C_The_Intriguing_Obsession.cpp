...
struct Hashing{
    ...
    int n;
    int primes;
    vector<lli> hashPrimes = {1000000009, 100000007,100000033};
    const lli base = 31;
...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0;i <= min(a,b);i++){
            w += (ncrf(a,i,x.first,x.second) * nprf(b,i,x.first,x.second))%MOD;
        }
        for(lli i = 0;i <= min(b,c);i++){
            y += (ncrf(b,i,x.first,x.second) * nprf(c,i,x.first,x.second))%MOD;
        }
        for(lli i = 0;i <= min(a,c);i++){
            z += (ncrf(a,i,x.first,x.second) * nprf(c,i,x.first,x.second))%MOD;
        }
        cout << (mod_mul(mod_mul(w,y,MOD),z,MOD))%MOD << endl;
    }
    return 0;
}