...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> a(n),b(m);
        lli c1= 0, c2 = 0;
        for(auto &i : a) {cin >> i;c1 += i;}
        for(auto &i : b) {cin >> i;c2 += i;}
        vector<lli> c,d;
        for(auto i : a) if(i != 0) c.push_back(i);
        for(auto i : b) if(i != 0) d.push_back(i);
        n = c.size();m = d.size();
        if(c1 != c2 || (c1 == c2 and n != m)){
            cout << "Alice" <<endl;
        }else{
            sort(c.begin(),c.end());
            sort(d.begin(),d.end());
