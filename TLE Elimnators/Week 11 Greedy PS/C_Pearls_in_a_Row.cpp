...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : a ) cin >> i;
        map<lli,lli> fc;
        vector<pair<lli,lli>> anss;
        lli l = 1, r= 0;
        lli ans = 0;
        for(auto i : a){
            r++;
            fc[i]++;
            if(fc[i] == 2) {ans++;fc.clear();anss.push_back({l,r});l = r+1;}
        }
        cout << (ans == 0 ? -1 : ans) << endl;
        if(ans != 0 and anss[anss.size() - 1].second != n) anss[anss.size() - 1].second = n;
        if(ans != 0) for(auto i : anss) cout << i.first << " " << i.second << endl;
    }
    return 0;
}