...
int main() {
    ...
    while(t--) {
        ...
        lli ops = 0,l=0,r=1;
        while(r < n){
            if((a[l].second - a[l].first + 1) == 0) {l++;r++;continue;}
            else if(a[r].second - a[r].first + 1 == 0) {ops++;a[l].first = 1;a[l].second = 0;}
            else if(a[l].second - a[l].first + 1 <= 2){
                if(a[l].first == a[r].first){
                    ops++;
                    a[l].first = 1;
                    a[l].second = 0;
                    a[r].first += 2;
                    if(a[r].first > a[r].second) {a[r].first = 1;a[r].second = 0;}
                }else if(a[l].first == a[r].second){
                    ops++;
                    a[l].first = 1;
                    a[l].second = 0;
                    a[r].second --;
                    if(a[r].first > a[r].second) {a[r].first = 1;a[r].second = 0;}
                }else if(a[l].second == a[r].first){
                    ops++;
                    a[l].first = 1;
                    a[l].second = 0;
                    if(a[l].first != a[l].second) a[r].first ++;
                    else a[r].first += 2;
                    if(a[r].first > a[r].second) {a[r].first = 1;a[r].second = 0;}
                    ...
                    a[r].second -= 2;
                    if(a[r].first > a[r].second) {a[r].first = 1;a[r].second = 0;}
                }else if(a[l].first + 1 == a[r].second){
                    ops++;
                    a[l].first = 1;
                    a[l].second = 0;
                    a[r].second -= 2;
        ...
        for(auto i : a) if((i.second - i.first + 1) != 0) {ops++;}
        cout << ops << endl;
    }
    return 0;
}