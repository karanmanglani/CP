...
int main() {
    ...
    while(t--) {
        ...
        lli l=0,r = 0;
        lli ans = 0;
        vector<pair<lli,pair<lli,bool>>> v;
        bool flag = false;
        while(l < n and r < n){
            while(l < n and a[l] == 0){l++;r = l;if(a[r]==2) flag = true;}
            while(r < n-1 and a[r+1] != 0) {r++;if(a[r]==2) flag = true;}
            if(l < n) v.push_back({l,{r,flag}});
            flag = false;
            l = ++r;
        }
        for(auto i : v){
            lli l = i.first;
            lli r = i.second.first;
            bool flag = i.second.second;
            ans++;
            if(flag){
                if(l-1 >= 0) ir[l-1] = true;
                if(r+1 < n) ir[r+1] = true;
            }else{
                if(l-1 >= 0 and !ir[l-1]) ir[l-1] = true;
                else if(r+1 < n and !ir[r+1]) ir[r+1] = true;
            }
            for(lli j = l;j <= r;j++){
                ir[j] = true;
            }
