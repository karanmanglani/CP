...
int main() {
    ...
    while(t--) {
        ...
        vector<lli> dp(n+1,-1);
        set<string> ans;
        // dp[i] => if it is possible to form a two or three lenght substring from here 
        auto f = [&](lli idx, auto &&F) -> lli {
            if(idx == n) return 1;
            if(idx > n) return 0;
            if(dp[idx] != -1) return dp[idx];
            dp[idx] = 0;
            // Take 2 length substring
            if(F(idx + 2, F)){
                if(idx + 2 == n){
                    ans.insert(s.substr(idx, 2));
                    dp[idx] = 1;
                }else{
                    if(idx + 4 <= n){
                        dp[idx] |= s.substr(idx, 2) != s.substr(idx + 2, 2);
                    }
                    if(idx + 5 <= n){
                        dp[idx] |= F(idx + 5,F);
                    }
                    if(dp[idx]) ans.insert(s.substr(idx, 2));
                }
            }

            // Take 3 length substring
            bool flag = false;
            if(F(idx + 3,F)){
                if(idx + 3 == n){
                    ans.insert(s.substr(idx, 3));
                    dp[idx] = 1;
                }else{
                    if(idx + 6 <= n){
                        flag |= s.substr(idx, 3) != s.substr(idx + 3, 3);
                    }
                    if(idx + 5 <= n){
                        flag |= F(idx + 5,F);
                    }
                    if(flag) ans.insert(s.substr(idx, 3));
                    dp[idx] |= flag;
                }
            }
