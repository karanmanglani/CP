...
int main() {
    ...
    while(t--) {
        ...
        lli li = -1,k = 0;
        for(lli i = 0;i < n - m;i++){
            if(li == -1) if(ans[i] > b[mni]) {ans[i] = b[mni];li = i;k = 1;}
            else if(k < m){
                ans[i] = b[(mni + k)%m];
                if(ans[i] > b[mni]) {ans[i] = b[mni];li = i;k = 0;}
                k++;
            }else {
                if(ans[i] > b[mni]) {ans[i] = b[mni];li = i;k = 1;}
            }
        }
        if(li != -1){
            for(lli j = 0;j < m;j++) ans[li+j] = b[(mni + j)%m];
        }

        lli i = n-m-1, j  = mni;
        while(i < n-1){
            if(ans[i] < b[j%m]) break;
            else if(ans[i] > b[j%m]){
                for(i = n-m,j=0;i < n;i++) {ans[i] = b[(j+mni)%m];j++;}
                break;
            }else if(ans[i] == b[j%m]){
                i++;
                j++;
