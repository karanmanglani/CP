...
int main() {
    ...
    while(t--) {
        ...
        for(auto &i : a) cin >> i;
        for(auto &i : b) cin >> i;
        vector<vector<lli>> dp(512,vector<lli>(n,1024));
        auto f = [&](lli mask, lli idx, auto &&F) -> lli{
            if(idx >= n) return 0;
            if(dp[mask][idx] != 1024) return dp[mask][idx];
            for(lli j = 0;j < m;j++){
                lli newMask = mask | (a[idx] & b[j]);
                dp[mask][idx] = min(dp[mask][idx] , F(newMask,idx + 1,F) | newMask);
            }
            return dp[mask][idx];
        };
