...
struct TrieNode {
    ...
    lli blockedStringsGoingBelow;
    TrieNode() {
        children.resize(letters, -1);
        stringsEndingHere = 0;
        stringsGoingBelow = 0;
        blockedStringsGoingBelow = 0;
    }
};
...
struct Trie {
    ...
    void add2(string &word) {
        ...
        for (char ch : word) {
            ...
            curr = trie[curr].children[ch - 'a'];
            trie[curr].stringsGoingBelow++;
            trie[curr].blockedStringsGoingBelow++;
        }
        trie[curr].stringsEndingHere++;
    }
    ...
    string solve(string s){
        ...
        for(char ch : s){
            curr = trie[curr].children[ch - 'a'];
            ans += ch;
            if(trie[curr].blockedStringsGoingBelow == trie[curr].stringsGoingBelow) return ans;
        }
        return "-1";
    }
};
...
int main() {
    ...
    while(t--) {
        char ch;cin >> ch;
        if(ch == '+'){string s;cin >> s;tr.add1(s);}
        else {string s;cin >> s;tr.add2(s);v.push_back(s);}
    }
    set<string> ans;
    
    for(auto i : v) ans.insert(tr.solve(i));
    for(auto i : ans) if(i == "-1"){
        cout << -1 << endl;
        return 0;
    }
    cout << ans.size() << endl;
