...
int main() {
    ...
    while(t--) {
        ...
        bool fnd = false;
        for(auto i : pf) {
            lli z = bem(i.first,i.second+1);
            if(i.second & 1) fnd = true;
            sm = (sm * ((((z - 1 + MOD) % MOD) * mi(i.first - 1)) % MOD)) % MOD;
            no = (no * ((i.second + 1) % MOD)) % MOD;
        }
        lli pro = 1;
        if(!fnd){
            for(auto i : pf){
                lli pwr = 1;
                for(auto i : pf){
                    pwr = (pwr * ((i.second + 1) % (MOD - 1))) % (MOD - 1); 
                }

                for(auto i : pf){
                    pro = (pro * bem(i.first, ((i.second/2) * pwr) % (MOD - 1))) % MOD;
                }
            }
        }else{
            lli pwr = 1;
            ...
            for(auto i : pf){
                pro = (pro * bem(i.first, (i.second * pwr) % (MOD - 1))) % MOD;
            }
        }
        cout << no << " " << sm << " "<< pro << endl;
    }
