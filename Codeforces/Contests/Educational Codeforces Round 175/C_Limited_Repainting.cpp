...
bool chk(vector<lli> &a, lli pen,string &s,lli k){
    lli ops = 0;
    lli n = a.size();
    lli r = 0;
    while(r < n and (s[r] == 'R' or (s[r] == 'B' and a[r] <= pen)) )r++;
    if(r != n) ops = 1;
    while(r < n){
        while(r < n and ((s[r] == 'R' and a[r] <= pen) or(s[r] == 'B')))r++;
        if(r == n) break;
        if(s[r] == 'R' and a[r] > pen){
            while(r < n and (s[r] == 'R' or (s[r] == 'B' and a[r] <= pen))) r++;
            if(r != n) ops++;
        }
        r++;
...
int main() {
    ...
    while(t--) {
        ...
        lli st = 0, e = *max_element(a.begin(),a.end());
        lli ans = n;
        while(st <= e) {
            lli m = st + (e - st) / 2;
            if(chk(a,m,s,k)) {ans = m;e = m-1;}
            else st = m + 1;
        }
        cout << ans << endl;
    }
