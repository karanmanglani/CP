...
struct TrieNode {
    TrieNode* children[2];
    lli max_index;
    TrieNode() : max_index(-1) {
        children[0] = children[1] = nullptr;
    }
};

void insert(TrieNode* root, lli num, lli index) {
    TrieNode* node = root;
    for (lli bit = 30; bit >= 0; --bit) {
        lli b = (num >> bit) & 1;
        if (!node->children[b]) {
            node->children[b] = new TrieNode();
        }
        node = node->children[b];
        node->max_index = max(node->max_index, index);
    }
}

lli finder(TrieNode* root, lli x, lli k) {
    TrieNode* node = root;
    lli best_j = -1;
    for (lli bit = 30; bit >= 0; --bit) {
        if (!node) break;
        ...
        if (k_bit) {
            ...
            lli desired_opp = 1 - x_bit;
            if (node->children[desired_opp]) {
                best_j = max(best_j, node->children[desired_opp]->max_index);
            }
    ...
    if (node) {
        best_j = max(best_j, node->max_index);
    }
    return best_j;
}

void clr(TrieNode* node) {
    if (!node) return;
    clr(node->children[0]);
    clr(node->children[1]);
    delete node;
