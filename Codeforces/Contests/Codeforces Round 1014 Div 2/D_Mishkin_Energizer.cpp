...
int main() {
    ...
    while(t--) {
        ...
        lli cntL = 0, cntI = 0, cntT = 0;
        for(auto c : arr){
            if(c=='L') cntL++;
            else if(c=='I') cntI++;
            else if(c=='T') cntT++;
        }
        auto eq = [&](lli a, lli b, lli c){ return a==b && b==c; };
        auto merger = [&](char a, char b) -> char {
            if((a=='L' && b=='I') || (a=='I' && b=='L')) return 'T';
            if((a=='L' && b=='T') || (a=='T' && b=='L')) return 'I';
            if((a=='I' && b=='T') || (a=='T' && b=='I')) return 'L';
            return '?';
        };
        if(eq(cntL, cntI, cntT)){
            cout << 0 << endl;
            continue;
        }
        if(arr.size() == 1 || (cntL == arr.size() || cntI == arr.size() || cntT == arr.size())){
            cout << -1 << endl;
            continue;
        }
        vector<lli> ops;
        bool done = false;
        for(lli it = 0; it <= 2 * n; it++){
            if(eq(cntL, cntI, cntT)){
                done = true;
                break;
            }
            char tar = (cntL <= cntI && cntL <= cntT) ? 'L' : (cntI <= cntL && cntI <= cntT) ? 'I' : 'T';
            lli pos = -1;
            for(lli j = 0; j < arr.size()-1; j++){
                if(arr[j] != arr[j+1] && merger(arr[j], arr[j+1]) == tar){
                    pos = j;
                    break;
                }
            }
            ...
            if(pos == -1) break;
            char nc = merger(arr[pos], arr[pos+1]);
            ops.push_back(pos+1);
            arr.insert(arr.begin()+pos+1, nc);
            if(nc=='L') cntL++;
            else if(nc=='I') cntI++;
            else if(nc=='T') cntT++;
        }
