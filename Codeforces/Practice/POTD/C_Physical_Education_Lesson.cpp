...
int main() {
    ...
    while(t--) {
        lli n,x;cin >> n >> x;
        unordered_set<lli> s,anss;
        for(lli i = 1;i*i <= n-x;i++){
            if((n-x)%i == 0){
                if(!(i&1)) s.insert(i);
                if(!(((n-x)/i)&1))s.insert((n-x)/i);
            }
        }
        for(lli i = 1;i*i <= n+x-2;i++){
            if((n+x-2)%i == 0){
                if(!(i&1)) s.insert(i);
                if(!(((n+x-2)/i)&1))s.insert((n+x-2)/i);
            }
        }
        lli ans = 0;
        for(auto i : s) anss.insert(1 + (i/2));
        for(auto i: anss) if(i >= x) ans++;
        cout << ans << endl;
    }
    return 0;
}