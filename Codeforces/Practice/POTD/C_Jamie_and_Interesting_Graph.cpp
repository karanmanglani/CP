...
int main() {
    ...
    while(t--) {
        lli n,m; cin >> n >> m;
        auto x = primeArr(1e6).second.second;
        vector<vector<pair<lli,lli>>> edges(n+1);
        lli sum = 0;
        lli edgCnt = 0;
        for(lli i = 2;i < n;i++){
            edges[i-1].push_back({i,1});
            edgCnt++;
            sum += 1;
        }

        lli nearestPrime = x[lower_bound(x.begin(), x.end(), sum+1) - x.begin()];
        if(nearestPrime == sum + 1){
            sum ++;
            edges[n-1].push_back({n,1});
            edgCnt++;
        }else{
            edges[n-1].push_back({n,nearestPrime - sum});
            sum = nearestPrime;
            edgCnt++;
        }
