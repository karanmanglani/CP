...
int main() {
    ...
    while(t--) {
        lli n;cin >> n;
        bool flag = false;
        vector<lli> a(n);for(auto &i : a) {cin >> i;if(i != 0) flag = true;}
        lli k;cin >> k;
        if(!flag){
            cout << -1 << endl;
            continue;
        }
        lli mxsm = accumulate(a.begin(),a.end(),0LL);
        lli ans = 0;
        while(k > mxsm){
            k -= mxsm;
            ans += n;
        }
        for(lli i = 0;i < n;i++) a.push_back(a[i]);
        n = a.size();
        lli l = 0, r = 0;
        lli sz =n+1;
        lli cs = 0;
        ...
        while(r < n){
            cs += a[r];
            while(l < r and (cs > k || a[l] == 0)) cs -= a[l++];
            if(cs == k) sz = min(sz,r-l+1);
            r++;
        }
        if(sz == n+1){
            cout << -1 << endl;
