...
int main() {
    ...
    while(t--) {
        ...
        vector<pair<lli,lli>> pws;
        for(auto monsters : caves){
            lli pw = 0, mnPw = 0;
            for(auto monster : monsters){
                if(pw <= monster){
                    lli diff = monster - pw + 1;
                    pw += diff;
                    mnPw += diff;
                }
                pw++;
            }
            pws.push_back({mnPw,monsters.size()});
        }

        sort(pws.begin(), pws.end() , [&](auto &x , auto &y){
