...
int main() {
    ...
    while(t--) {
        string s;cin >> s;
        lli n = s.length();
        vector<vector<lli>> dp(n,vector<lli> (5,-1));
        auto f = [&](lli idx, lli stp , auto &&F) -> lli{
            if(idx == n){
                return 0;
            }
            if(dp[idx][stp] != -1) return dp[idx][stp];
            dp[idx][stp] = 0;

            // Do not take this 
            dp[idx][stp] += F(idx + 1, stp,F);

            // Take this
            bool canTake = ((stp == 2 && s[idx] == 'o' )|| (stp != 2 && s[idx] == 'v' && idx+1 < n && s[idx+1] == 'v'));
            if(canTake){
                if(stp == 3){
                    dp[idx][stp] += 1;
                }else{
                    if(stp == 2) dp[idx][stp] += F(idx + 1, stp + 1,F);
                    else dp[idx][stp] += F(idx + 2, stp + 2, F);
                }
            }
