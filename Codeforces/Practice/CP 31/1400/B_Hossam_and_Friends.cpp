...
int main() {
    ...
    while(t--) {
        lli n,m;cin >> n >> m;
        vector<vector<lli>> notFriends(n + 1);
        vector<lli> notFriendOf(n+1,0);

        for(lli i = 0;i < m;i++){
            lli a,b;cin >> a >> b;
            notFriends[a].push_back(b);
            notFriends[b].push_back(a);
        }

        lli l = 1, r = 1;
        lli ans = 0;
        while(r <= n){
            while(notFriendOf[r] > 0){
                for(auto i : notFriends[l]){
                    notFriendOf[i]--;
                }
                l++;
            }
            ans += (r-l+1);
            for(auto i : notFriends[r]){
                notFriendOf[i]++;
            }
            r++;
        }
