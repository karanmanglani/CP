...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0;i < n;i++)a[b[i].first].push_back(b[i].second);
        for(lli i = 0;i < n;i++)sort(a[i].rbegin(), a[i].rend());
        vector<vector<lli>> psum(n, vector<lli>());
        for(lli i = 0;i < n;i++){
            lli sm = 0;
