...
int main() {
    ...
    while(t--) {
        ...
        auto f = [&](lli idx, lli row, auto&& F) -> lli{
            if(idx >= n) return 0;
            if(dp[row][idx] != -1) return dp[row][idx];

            dp[row][idx] = 0;
            // Take the current one
            dp[row][idx] = max(dp[row][idx], a[row][idx] + F(idx + 1, row ^ 1, F));

            // Skip the current one
            dp[row][idx] = max(dp[row][idx], F(idx + 1, row, F));
            return dp[row][idx];
        };

        cout << max(f(0, 0, f), f(0, 1, f)) << endl;
    }
    return 0;
}