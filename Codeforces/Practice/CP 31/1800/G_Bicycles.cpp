...
int main() {
    ...
    while(t--) {
        ...
        st.insert({0,{1,s[0]}});
        vector<lli> dis(n,LLONG_MAX);
        vector<lli> bm(n,LLONG_MAX);
        dis[0] = 0;
        bm[0] = s[0];
        while(!st.empty()){
            auto x = *(st.begin());
            lli dist = x.first;
            lli node = x.second.first;
            lli bmin = x.second.second;
            st.erase(st.begin());
            if(s[node-1] < bmin) bmin = s[node-1];
            bm[node-1] = min(bmin,bm[node-1]);
            if(node == n){
                cout << dist << endl;
                break;
            }
            for(auto i : adj[node]){
                ...
                lli w = i.second;
                lli dst = w*bmin + dist;
                if(dst < dis[v-1] || bmin < bm[v-1]){
                    if(dst < dis[v-1]) dis[v-1] = dst;
                    st.insert({dst,{v,bmin}});
                }
            } 
        }
    }
