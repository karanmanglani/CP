...
int main() {
    ...
    while(t--) {
        ...
        sort(a.begin(), a.end());
        map<lli,lli> fc;
        vector<lli> pfx(n+1,0), sfx(n+1,0);
        for(lli i = 0; i < n; i++) {
            pfx[i + 1] = pfx[i] + a[i];
            fc[a[i]]++;
        }
        ...
        lli ans = LLONG_MAX;
        auto makeIt = [&](lli idx) -> lli{
            if(fc[a[idx]] >= k) return 0;
            lli lsum = pfx[idx];
            lsum = (a[idx] * idx) - lsum;
            lli rsum = sfx[idx + 1];
            rsum -= ((n - idx - 1) * a[idx]);
            lli lel = idx, rel = n - idx - 1;
            if(lel + fc[a[idx]] >= k && rel + fc[a[idx]] >= k ){
                return min(lsum,rsum);
            }else if(lel + fc[a[idx]] >= (k)){
                return lsum;
            }else if(rel + fc[a[idx]] >= (k)){
                return rsum;
            }else return lsum + rsum;
        };

        for(lli i = 0;i < n;i++){
            ans = min(ans, makeIt(i));
        }
        cout << ans << endl;
    }
    return 0;
