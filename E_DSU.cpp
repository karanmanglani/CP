...
int main() {
    ...
    while(t--) {
        ...
        for(lli i = 0; i < m;i++){
            ...
            if(md != 0) cost = min(abs(best[md]-a[i]),abs(best[md-1]-a[i]));
            else cost = abs(best[md]-a[i]);

            if(cost == 0) st.insert({cost + b[i],{0,a[i]}});
            else st.insert({cost + b[i],{1,a[i]}});
        }
        lli ans = 0;
        auto it = st.begin();
        while(k--){
